...
/** \brief This function waits till all the cores have set their corresponding bits in the event. This function along with IfxCpu_emitEvent() are used to achieve the synchronisation between all the cores.
 ...
 * IfxCpu_waitEvent(&event, timeoutMs); // timeoutMs is timeout value to wait
 *
 * \endcode
 *
 */
IFX_EXTERN void IfxCpu_waitEvent(IfxCpu_syncEvent *event, uint32 timeout);
