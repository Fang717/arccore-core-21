...
StatusType Os_ArchGetProtectionType( uint32 exception ) {
    ...
    if( (exception == EXCEPTION_IVOR1) || (exception == EXCEPTION_IVOR2) || (exception == EXCEPTION_IVOR13 ) ) {
        /* @req SWS_Os_00044
    ...
    } else if (exception <= 14 ) {
        /* Map most exceptions to E_OS_PROTECTION_EXCEPTION */
...
void Os_ArchSetupContext( OsTaskVarType *pcbPtr ) {
    Os_ExceptionFrameType *excPtr = (Os_ExceptionFrameType *)((uint8 *)pcbPtr->stack.curr + sizeof(Os_IsrFrameType)) ;
...
void Os_ArchInit( void ) {
    ...
    for(uint32 i=0;i<OS_APPLICATION_CNT;i++) {
        ...
        aVarP->regs[MPU0_REG_TASK_DATA_END] = ((uint32)aConstP->dataEnd - 1UL);

        /* Data, start and stop */
        aVarP->regs[MPU0_REG_TASK_BSS_START] = (uint32_t)aConstP->bssStart;
        aVarP->regs[MPU0_REG_TASK_BSS_END] = ((uint32)aConstP->bssEnd - 1UL);
    }
#endif
