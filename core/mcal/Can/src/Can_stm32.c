...
Can_UnitType CanUnits[CAN_CONTROLLER_CNT] =
{
    {
...
Can_ReturnType Can_Write( Can_HwHandleType Hth, const Can_PduType *PduInfo ) {
    ...
    VALIDATE( (PduInfo->length <= 8), CAN_WRITE_SERVICE_ID, CAN_E_PARAM_DATA_LENGTH,CAN_NOT_OK );
...
void Can_MainFunction_Mode( void )
{
    ...
    for (int configId=0; configId < CAN_ARC_CTRL_CONFIG_CNT; configId++) {
        ...
        //Get the controller from config index and index CanUnits
        const Can_ControllerConfigType * canHwConfig = GET_CONTROLLER_CONFIG(configId);
        uPtr = &CanUnits[canHwConfig->CanControllerId];
