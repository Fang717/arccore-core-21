...
void Can_Init(const Can_ConfigType *Config) {
    ...
    /* @req 4.1.2|4.3.0/SWS_Can_00053 Can_Init shall not change registers of CAN controller Hardware resources that are not used.*//* Don't change registers for controllers not used. */
    /* @req 4.1.2|4.3.0/SWS_Can_00239 The function Can_Init shall initialize all on-chip hardware resources that are used by the CAN controller. The only exception to this is the digital I/O pin configuration (of pins used by CAN), which is done by the port driver.*//* Init all on chip hardware */
    /* @req 4.1.2|4.3.0/SWS_Can_00250 The function Can_Init shall initialize several contents*//* Initialize variables, init controllers. */
    ...
    /* @req 4.1.2|4.3.0/SWS_Can_00174 If default error detection for the Can module is enabled: The function Can_Init shall raise the error CAN_E_TRANSITION if the driver is not in state CAN_UNINIT.*/
    /* @req 4.1.2/SWS_Can_00175 */
    /* @req 4.1.2|4.3.0/SWS_Can_00223 void Can_Init( const Can_ConfigType* Config )*/

    uint8 hwCh = 0x0u;

    /* DET Error Check */
    if ( E_OK != validate((boolean) (Can_Global.initRun == CAN_UNINIT), CAN_INIT_SERVICE_ID, CAN_E_TRANSITION)) {
        return;
    }
    ...
    for (uint8 ch = 0; ch < CAN_ARC_CTRL_CONFIG_CNT; ch++) {
        hwCh = ((uint8) Can_Global.config->CanConfigSetPtr->CanController[ch].CanHwUnitId);
    ...
    /** @req 4.1.2|4.3.0/SWS_Can_00246 The function Can_Init shall change the module state to CAN_READY, after initializing all controllers inside the HW Unit.*//* Should be done after initializing all controllers */
...
void Can_GetVersionInfo( Std_VersionInfoType* versioninfo) {
    ...
    /** @req 4.1.2|4.3.0/SWS_Can_00224 void Can_GetVersionInfo( Std_VersionInfoType* versioninfo )*/
    /** @req 4.1.2|4.3.0/SWS_Can_00177 If default error detection for the Can module is enabled: The function Can_GetVersionInfo shall raise the error CAN_E_PARAM_POINTER if the parameter versionInfo is a null pointer.*/
...
Can_ReturnType Can_SetControllerMode( uint8 Controller, Can_StateTransitionType Transition )
{
    ...
    /** @req 4.1.2|4.3.0/SWS_Can_00017 The function Can_SetControllerMode shall perform software triggered state transitions of the CAN controller State machine.*/
    /** @req 4.1.2|4.3.0/SWS_Can_00230 Can_ReturnType Can_SetControllerMode( uint8 Controller, Can_StateTransitionType Transition )*/
    /** @req 4.1.2|4.3.0/SWS_Can_00199 If default error detection for the Can module is enabled: if the parameter Controller is out of range, the function Can_SetControllerMode shall raise default error CAN_E_PARAM_CONTROLLER and return CAN_NOT_OK.*/
    ...
    /** @req 4.1.2|4.3.0/SWS_Can_00198 If default error detection for the Can module is enabled: if the module is not yet initialized, the function Can_SetControllerMode shall raise default error CAN_E_UNINIT and return CAN_NOT_OK.*/
    if( E_OK != validate((boolean) (Can_Global.initRun == CAN_READY), CAN_SETCONTROLLERMODE_SERVICE_ID, CAN_E_UNINIT) ) {
        return CAN_NOT_OK;
    }

    switch (Transition) {
    case CAN_T_START:
        /* @req 4.0.3/CAN409 *//** @req 4.1.2|4.3.0/SWS_Can_00409 The function Can_SetControllerMode(CAN_CS_STARTED) shall set the hardware registers in a way that makes the CAN controller participating on the network */
        /** @req 4.1.2|4.3.0/SWS_Can_00261 The function Can_SetControllerMode(CAN_CS_STARTED) shall set the hardware registers in a way that makes the CAN controller participating on the network*/
        validate_rv = validate((boolean)(Can_Unit[ch].state == CANIF_CS_STOPPED), CAN_SETCONTROLLERMODE_SERVICE_ID, CAN_E_TRANSITION);
        if( validate_rv == E_OK ) {
            retVal = can_Transition( ch, CANIF_CS_STARTED,  Transition );

            /* @req 4.1.2|4.3.0/SWS_Can_00196 The function Can_SetControllerMode shall enable interrupts that are needed in the new state.*/
            /* @req 4.1.2|4.3.0/SWS_Can_00425 Enabling of CAN interrupts shall not be executed, when CAN interrupts have been disabled by function Can_DisableControllerInterrupts.*/

            /* Call Can_Hw_EnableControllerInterrupts instead of  Can_EnableControllerInterrupts so as not to  change the lock counter. */
            if( Can_Unit[ch].lockCnt == 0u ) {
                Can_Hw_EnableControllerInterrupts(hwCh);
            }
        }
        break;
    case CAN_T_WAKEUP:
        validate_rv = validate( (boolean)((Can_Unit[ch].state == CANIF_CS_SLEEP) || (Can_Unit[ch].state == CANIF_CS_STOPPED)), CAN_SETCONTROLLERMODE_SERVICE_ID, CAN_E_TRANSITION );
        if( validate_rv == E_OK ) {
    ...
    case CAN_T_STOP:
        ...
        validate_rv = validate( (boolean)((Can_Unit[ch].state == CANIF_CS_STARTED) || (Can_Unit[ch].state == CANIF_CS_STOPPED)), 0x3, CAN_E_TRANSITION );
        if( validate_rv == E_OK ) {
            retVal = can_Transition( ch, CANIF_CS_STOPPED,  Transition );

            /* @req 4.1.2|4.3.0/SWS_Can_00426 Disabling of CAN interrupts shall not be executed, when CAN interrupts have been disabled by function Can_DisableControllerInterrupts.*/
            if( Can_Unit[ch].lockCnt == 0u ) {
                Can_Hw_DisableControllerInterrupts(hwCh);
            }
        }
        break;
    case CAN_T_SLEEP:
        /* @req 4.1.2|4.3.0/SWS_Can_00411 When the function Can_SetControllerMode(CAN_CS_SLEEP) is entered and the CAN controller is neither in state STOPPED nor in state SLEEP, it shall detect a invalid state transition (Compare to SWS_Can_00200).*/
        ...
        /* @req 4.1.2|4.3.0/SWS_Can_00265 The function Can_SetControllerMode(CAN_CS_SLEEP) shall set the controller into sleep mode.*/

        validate_rv = validate((boolean)(Can_Unit[ch].state == CANIF_CS_STOPPED), 0x3, CAN_E_TRANSITION );
        if( E_OK == validate_rv ) {
            retVal = can_Transition( ch, CANIF_CS_STOPPED,  Transition );
        }
        ...
        break;
    default:
        /* @req 4.1.2|4.3.0/SWS_Can_00200 If default error detection for the Can module is enabled: if an invalid transition has been requested, the function Can_SetControllerMode shall raise the error CAN_E_TRANSITION and return CAN_NOT_OK*/
#if (CAN_DEV_ERROR_DETECT == STD_ON)
...
void Can_DeInit(void) {
    /** @req 4.3.0/SWS_Can_91002 void Can_DeInit( void )*/

    /** @req 4.3.0/SWS_Can_91011 The function Can_DeInit shall raise the error CAN_E_TRANSITION if the driver is not in state CAN_READY */
    if( E_OK != validate((boolean) (Can_Global.initRun == CAN_READY), CAN_DEINIT_SERVICE_ID, CAN_E_TRANSITION) ) {
        return;
    }

    for (uint8 ch = 0; ch < CAN_ARC_CTRL_CONFIG_CNT; ch++) {

        Can_Hw_DisableControllerInterrupts(ChannelToHwChannel(ch));

        /** @req 4.3.0/SWS_Can_91012 If development error detection for the Can module is enabled: The function Can_DeInit shall raise the error CAN_E_TRANSITION if any of the CAN controllers is in state STARTED.*/
        if( E_OK != validate((boolean) ((Can_Unit[ch].state) != CANIF_CS_STARTED), CAN_DEINIT_SERVICE_ID, CAN_E_TRANSITION) ) {
            return;
        }

        Can_Unit[ch].lockCnt = 0;
        /** @req 4.3.0/SWS_Can_91009 The function Can_DeInit shall change the module state to CAN_UNINIT before de-initializing all controllers inside the HW unit*/
        /** @req 4.3.0/SWS_Can_91010 The function Can_DeInit shall set all CAN controllers in the state UNINIT*/
...
void Can_DisableControllerInterrupts(uint8 Controller) {
    /** @req 4.1.2|4.3.0/SWS_Can_00206 If default error detection for the Can module is enabled: The function Can_DisableControllerInterrupts shall raise the error CAN_E_PARAM_CONTROLLER if the parameter Controller is out of range.*/
    /** @req 4.1.2|4.3.0/SWS_Can_00049 The function Can_DisableControllerInterrupts shall access the CAN controller registers to disable all interrupts for that CAN controller only, if interrupts for that CAN Controller are enabled.*/
    /** @req 4.1.2|4.3.0/SWS_Can_00202 When Can_DisableControllerInterrupts has been called several times, Can_EnableControllerInterrupts must be called as many times before the interrupts are re-enabled.*/
    /** @req 4.1.2|4.3.0/SWS_Can_00205 If default error detection for the Can module is enabled: The function Can_DisableControllerInterrupts shall raise the error CAN_E_UNINIT if the driver not yet initialized.*/
    /** @req 4.1.2|4.3.0/SWS_Can_00231 void Can_DisableControllerInterrupts( uint8 Controller )*/
    ...
    /** @req 4.1.2|4.3.0/SWS_Can_00209 If default error detection for the Can module is enabled: The function Can_EnableControllerInterrupts shall raise the error CAN_E_UNINIT if the driver not yet initialized.*/
    validate_rv = validate((boolean) (Can_Global.initRun == CAN_READY), CAN_DISABLECONTROLLERINTERRUPTS_SERVICE_ID, CAN_E_UNINIT );
    if ( E_OK != validate_rv) {
        return;
    }
...
void Can_EnableControllerInterrupts(uint8 Controller) {
    /** @req 4.1.2|4.3.0/SWS_Can_00210 If default error detection for the Can module is enabled: The function Can_EnableControllerInterrupts shall raise the error CAN_E_PARAM_CONTROLLER if the parameter Controller is out of range.*/
    /** @req 4.1.2|4.3.0/SWS_Can_00050 The function Can_EnableControllerInterrupts shall enable all interrupts that must be enabled according the current software status.*/
    /** @req 4.1.2|4.3.0/SWS_Can_00232 void Can_EnableControllerInterrupts( uint8 Controller )*/
    ...
    validate_rv = validate((boolean) (Can_Global.initRun == CAN_READY), CAN_ENABLECONTROLLERINTERRUPTS_SERVICE_ID, CAN_E_UNINIT );
    if ( E_OK != validate_rv) {
        return;
    }
#endif
    ...
    if (Can_Unit[Controller].lockCnt == 0) {
        /** @req 4.1.2|4.3.0/SWS_Can_00208 The function Can_EnableControllerInterrupts shall perform no action when Can_DisableControllerInterrupts has not been called before.*/
        /* Do nothing */
    } else {
        Can_Unit[Controller].lockCnt--;
        if (Can_Unit[Controller].lockCnt == 0) {
...
Can_ReturnType Can_Write(Can_HwHandleType Hth, const Can_PduType *PduInfo) {
    ...
    /** @req 4.1.2|4.3.0/SWS_Can_00233 Can_ReturnType Can_Write( Can_HwHandleType Hth, const Can_PduType* PduInfo )*/
    /** @req 4.1.2|4.3.0/SWS_Can_00212 The function Can_Write shall perform following actions if the hardware transmit object is free:*/
    /** @req 4.1.2|4.3.0/SWS_Can_00011 The Can module shall directly copy the data from the upper layer buffers. It is the responsibility of the upper layer to keep the buffer consistent until return of function call (Can_Write).*/
    /** @req 4.1.2|4.3.0/SWS_Can_00275 The function Can_Write shall be non-blocking.*/
	/** @req 4.1.2|4.3.0/SWS_Can_00216 If default error detection for the Can module is enabled: The function Can_Write shall raise the error CAN_E_UNINIT and shall return CAN_NOT_OK if the driver is not yet initialized.*/
    ...
    validate_rv = validate((boolean)(Can_Global.initRun == CAN_READY), CAN_WRITE_SERVICE_ID, CAN_E_UNINIT);
    if ( E_OK != validate_rv) {
        return CAN_NOT_OK;
    }
    /** @req 4.1.2|4.3.0/SWS_Can_00219 If default error detection for CanDrv is enabled: Can_Write() shall raise CAN_E_PARAM_POINTER and shall return CAN_NOT_OK if the parameter PduInfo is a null pointer.*/
    ...
    /** @req 4.1.2|4.3.0/SWS_Can_00218 The function Can_Write shall return CAN_NOT_OK and if default error detection for the CAN module is enabled shall raise the error CAN_E_PARAM_DATA_LENGTH:*/
    validate_rv = validate((boolean)(PduInfo->length <= 8), CAN_WRITE_SERVICE_ID, CAN_E_PARAM_DATA_LENGTH);
    if ( E_OK != validate_rv) {
        return CAN_NOT_OK;
    }
    /** @req 4.1.2|4.3.0/SWS_Can_00217 If default error detection for the Can module is enabled: The function Can_Write shall raise the error CAN_E_PARAM_HANDLE and shall return CAN_NOT_OK if the parameter Hth is not a configured Hardware Transmit Handle.*/
...
Std_ReturnType Can_ChangeBaudrate(uint8 Controller, const uint16 Baudrate) {
    ...
    validate_rv = validate((boolean)(Can_Global.initRun == CAN_READY), CAN_CHANGE_BAUD_RATE_SERVICE_ID, CAN_E_UNINIT);
    if ( E_OK != validate_rv) {
        return E_NOT_OK;
    }
    ...
    validate_rv = validate((boolean)(Can_CheckBaudrate(Controller, Baudrate) == E_OK), CAN_CHANGE_BAUD_RATE_SERVICE_ID, CAN_E_PARAM_BAUDRATE);
    if ( E_OK != validate_rv) {
        return E_NOT_OK;
    }

	/** @req 4.1.2/SWS_Can_00453 */
    validate_rv = validate((boolean)(Can_Unit[Controller].state==CANIF_CS_STOPPED), CAN_CHANGE_BAUD_RATE_SERVICE_ID, CAN_E_TRANSITION);
    if ( E_OK != validate_rv) {
        return E_NOT_OK;
    }
#endif

    const Can_ControllerConfigType *config = &Can_Global.config->CanConfigSetPtr->CanController[Controller];
...
Std_ReturnType Can_CheckBaudrate(uint8 Controller, const uint16 Baudrate) {
    ...
    validate_rv = validate((boolean)(Can_Global.initRun == CAN_READY), CAN_CHECK_BAUD_RATE_SERVICE_ID, CAN_E_UNINIT);
    if ( E_OK != validate_rv) {
        return E_NOT_OK;
    }

    /** @req 4.1.2/SWS_Can_00457 */
    validate_rv = validate((boolean)((Can_Global.configured & (1uL << Controller)) == (1uL << Controller)) , CAN_CHECK_BAUD_RATE_SERVICE_ID, CAN_E_PARAM_CONTROLLER );
    if ( E_OK != validate_rv) {
        return E_NOT_OK;
    }
...
#endif
    const Can_ControllerConfigType *ctrlConfig = &Can_Global.config->CanConfigSetPtr->CanController[Controller];
    validate_rv = E_NOT_OK; // Reset, otherwise validate_rv is always E_OK
...
void Can_MainFunction_Write( void )
{
    ...
    validate_rv = validate((boolean)(Can_Global.initRun == CAN_READY), CAN_MAINFUNCTION_WRITE_SERVICE_ID, CAN_E_UNINIT );
    if ( E_OK != validate_rv) {
        return;
    }
...
void Can_MainFunction_Read( void )
{
    ...
    validate_rv = validate((boolean)(Can_Global.initRun == CAN_READY), CAN_MAINFUNCTION_READ_SERVICE_ID, CAN_E_UNINIT );
    if ( E_OK != validate_rv) {
        return;
    }
...
void Can_MainFunction_BusOff( void )
{
    ...
    validate_rv = validate((boolean)(Can_Global.initRun == CAN_READY), CAN_MAINFUNCTION_BUSOFF_SERVICE_ID, CAN_E_UNINIT );
    if ( E_OK != validate_rv) {
        return;
    }
...
void Can_MainFunction_Wakeup( void )
{
    ...
    validate_rv = validate((boolean)(Can_Global.initRun == CAN_READY), CAN_MAINFUNCTION_WAKEUP_SERVICE_ID, CAN_E_UNINIT );
    if ( E_OK != validate_rv) {
        return;
    }
...
void Can_Arc_MainFunction_Error(void)
{
    ...
    validate_rv = validate((boolean)(Can_Global.initRun == CAN_READY), CAN_MAINFUNCTION_ERROR_SERVICE_ID, CAN_E_UNINIT );
    if ( E_OK != validate_rv) {
        return;
    }
...
void Can_MainFunction_Mode(void) {
    /** @req 4.1.2|4.3.0/SWS_Can_00368 void Can_MainFunction_Mode( void )*//* API */
    /** @req 4.1.2|4.3.0/SWS_Can_00369 The function Can_MainFunction_Mode shall implement the polling of CAN status register flags to detect transition of CAN Controller state. Compare to chapter 7.3.2.*/
    /** @req 4.1.2/SWS_Can_00379 */
    Can_Arc_ModeType hwMode;
    CanIf_ControllerModeType canIfMode = CANIF_CS_UNINIT;

#if (CAN_DEV_ERROR_DETECT == STD_ON)
    if( E_OK !=  validate((boolean) (Can_Global.initRun == CAN_READY), CAN_MAIN_FUNCTION_MODE_SERVICE_ID, CAN_E_UNINIT ) ) {
        return;
    }
#endif
    ...
    for (uint8 ch = 0; ch < CAN_ARC_LOG_UNIT_CNT; ch++) {
        ...
        if ( FALSE == Can_Unit[ch].transitonPending) {
            continue;
        }
        ...
        if (canIfMode != CANIF_CS_UNINIT) {
            /** @req 4.1.2|4.3.0/SWS_Can_00373 The function Can_Mainfunction_Mode shall call the function CanIf_ControllerModeIndication to notify the upper layer about a successful state transition of the corresponding CAN controller referred by abstract CanIf ControllerId, in case the state transition was triggered by function Can_SetControllerMode.*/
        ...
        if (clearPending == TRUE) {
            Can_Unit[ch].transitonPending = FALSE;
        }
    }
}
