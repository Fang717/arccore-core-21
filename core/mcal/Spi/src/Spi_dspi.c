...
/** @tagSettings DEFAULT_ARCHITECTURE=PPC|MPC5607B|MPC5646B|MPC5645S|MPC5777M|MPC5777C|MPC5748G */

/** @req SWS_Spi_00174 - Dem_EventIdType shall be imported from Dem_Types.h */
/** @req SWS_Spi_00138 - The function Spi_ReadIB is pre-compile time configurable by the parameter SpiChannelBuffersAllowed. This function is only relevant for Channels with IB.*/
/** @req SWS_Spi_00016 - The function Spi_ReadIB shall read synchronously one or more data from an IB SPI Handler/Driver Channel specified by the respective parameter. (SRS_Spi_12099, SRS_Spi_12152)*/
/** @req SWS_Spi_00179 - Spi_ReadIB*/
/** @req SWS_Spi_00315 - The function Spi_ReadIB provides the service for reading synchronously one or more data from an IB SPI Handler/Driver Channel specified by parameter.*/
/** @req SWS_Spi_00131 */
/** @req SWS_Spi_00312 - The operation Spi_ReadIB is Re-entrant.*/
/** @req SWS_Spi_00150 - The type Spi_AsyncModeType is made available or not depending on the pre-compile time parameter: SpiLevelDelivered. This is only relevant for LEVEL 2.*/
/** @req SWS_Spi_00313 - The function Spi_ReadIB return values E_OK: read command has been accepted.*/
/** @req SWS_Spi_00035 - When the function Spi_SyncTransmit is called while a se-quence is on transmission and SPI_SUPPORT_CONCURRENT_SYNC_TRANSMIT is disabled or another sequence is on transmition on same bus,
...
/** @req SWS_Spi_00027 - The SPI Handler/Driver's environment shall call the function Spi_ReadIB after a Transmit method call to have relevant data within IB Channel.*/
/** @req SWS_Spi_00314 - The function Spi_ReadIB return values E_NOT_OK: read command has not been accepted.*/
/** @req SWS_Spi_00140 - If SpiHwUnitSynchronous is set to "Synchronous" for a job, the associated bus defined by SpiHwUnit behave same as prearranged bus.
 * It means that all requirements valid for prearranged bus will be valid also for the bus assigned to this job. */
...
#if !defined(CFG_DRIVERS_USE_CONFIG_ISRS) && !defined(SPI_ARC_ISR_APP)
#if ((OS_SC3 == STD_ON) || (OS_SC4 == STD_ON))
#error You must define SPI_ARC_ISR_APP
#else
#define SPI_ARC_ISR_APP             0
#endif
#endif
...
#if defined(CFG_MPC5744P) || defined(CFG_MPC5643L) || defined(CFG_SPC56XL70) || defined(CFG_MPC5777M) || defined(CFG_MPC5777C) || defined(CFG_MPC5645S) || defined(CFG_MPC5748G) || defined(CFG_MPC5746C) || defined(CFG_MPC5747C)
...
static inline DSPI_Type * GetSpiHwPtr(uint32 unit) {
    ASSERT(unit<4 );
    /*lint -e{661} LINT: ASSERT exist for array out of bound error*/
    return SPI_HW[unit];
}
...
static DSPI_Type * const SPI_HW[] = {
        (DSPI_Type *)&DSPI_0, (DSPI_Type *) &DSPI_1,  (DSPI_Type *) &DSPI_2, (DSPI_Type *) &DSPI_3,(DSPI_Type *) &DSPI_4
};
...
static inline void EnableEoqInterrupt( DSPI_Type *hwPtr) {
    /*lint -e{1776} OTHER required operation */
    ASSERT(hwPtr!=NULL_PTR);
    hwPtr->RSER.B.EOQF_RE = 1u;
}

static inline void DisableEoqInterrupt( DSPI_Type *hwPtr) {
    /*lint -e{1776} OTHER required operation */
    ASSERT(hwPtr!=NULL_PTR);
    hwPtr->RSER.B.EOQF_RE = 0u;
}
...
static sint32 checkJobTotalLength(Spi_UnitType *uPtr, uint16_t *numberOfChannels)
{
    ...
    ASSERT(numberOfChannels!=NULL_PTR);
...
sint32 Spi_Hw_Rx( Spi_UnitType *uPtr ) {
    ASSERT(uPtr!=NULL_PTR && uPtr->pData!=NULL_PTR);
...
sint32 Spi_Hw_Rx(Spi_UnitType *uPtr) {
    /*lint -e{1776} OTHER required operation */
    ASSERT((uPtr!=NULL_PTR) && (uPtr->jobPtr!=NULL_PTR) && (uPtr->pData!=NULL_PTR));
    ...
    do {
        ...
        if (((uint32)rxLength - uPtr->pData->rxDataUnitsCopied) == 0) {
            ...
            uPtr->rxChIdxInJob++;
            if ( uPtr->jobPtr->SpiChannelAssignment[uPtr->rxChIdxInJob] == NULL_PTR ) { /*lint !e9032 Comparison to NULL_PTR is ok */
                uPtr->rxChIdxInJob = 0;
                rv = SPIE_OK;
                break;
            }
...
{
    /*lint -e{1776} OTHER required operation */
    ASSERT((uPtr!=NULL_PTR) && (uPtr->jobPtr!=NULL_PTR) && (uPtr->pData!=NULL_PTR) && (uPtr->jobPtr->SpiDeviceAssignment!=NULL_PTR));
    Spi_CommandType                 cmd;
    ...
    do {
        ...
        if( 0 == (((uint32)bufLenDataUnits) - uPtr->pData->txDataUnitsCopied)) {
               ...
               uPtr->txChIdxInJob++;
               if ( uPtr->jobPtr->SpiChannelAssignment[uPtr->txChIdxInJob] == NULL_PTR ) { /*lint !e9032 Comparison to NULL_PTR is ok */
                   uPtr->txChIdxInJob = 0;
...
#if defined (CFG_MPC5748G) || defined(CFG_MPC5747C) || defined(CFG_MPC5777C)
                                   ...
                                   const Spi_ChannelConfigType *chPtr )
{
    ...
    /*lint -e{656} LINT: "This is considered a very minor deviation from the
...
static void CtarSetup(void) {
    ...
    for (Spi_JobType j = 0; j < SPI_MAX_JOB; j++) {
        ...
        /* Loop over channels */
        for (uint32 chIdx = 0; (chPtr = jobPtr->SpiChannelAssignment[chIdx])!= NULL_PTR; chIdx++) {
...
void Spi_Hw_InitController(uint32 hwUnit ) {
    ...
    #if (SPI_USE_HW_UNIT_0 == STD_ON )
        case 0:
            ISR_INSTALL_ISR2("SPI_A", Spi_Isr_A, DSPI_A_ISR_EOQF, 15, SPI_ARC_ISR_APP ); /*lint !e641 Converting enum to int*/
            break;
    #endif
    #if (SPI_USE_HW_UNIT_1 == STD_ON )
            case 1:
            ISR_INSTALL_ISR2("SPI_B",Spi_Isr_B, DSPI_B_ISR_EOQF, 15, SPI_ARC_ISR_APP ); /*lint !e641 Converting enum to int*/
            break;
    #endif
    #if (SPI_USE_HW_UNIT_2 == STD_ON )
            case 2:
            ISR_INSTALL_ISR2("SPI_C",Spi_Isr_C, DSPI_C_ISR_EOQF, 15, SPI_ARC_ISR_APP ); /*lint !e641 Converting enum to int*/
            break;
    #endif
    #if (SPI_USE_HW_UNIT_3 == STD_ON )
            case 3:
            ISR_INSTALL_ISR2("SPI_D",Spi_Isr_D, DSPI_D_ISR_EOQF, 15, SPI_ARC_ISR_APP ); /*lint !e641 Converting enum to int*/
            break;
    #endif
    #if (SPI_USE_HW_UNIT_4 == STD_ON )
            case 4:
            ISR_INSTALL_ISR2("SPI_E",Spi_Isr_E, DSPI_E_ISR_EOQF, 15, SPI_ARC_ISR_APP ); /*lint !e641 Converting enum to int*/
            break;
    #endif
    #if (SPI_USE_HW_UNIT_5 == STD_ON )
            case 5:
            ISR_INSTALL_ISR2("SPI_F",Spi_Isr_F, DSPI_F_ISR_EOQF, 15, SPI_ARC_ISR_APP ); /*lint !e641 Converting enum to int*/
            break;
    #endif
    #if (SPI_USE_HW_UNIT_6 == STD_ON )
            case 5:
            ISR_INSTALL_ISR2("SPI_G",Spi_Isr_F, DSPI_G_ISR_EOQF, 15, SPI_ARC_ISR_APP ); /*lint !e641 Converting enum to int*/
            break;
    #endif
    #if (SPI_USE_HW_UNIT_7 == STD_ON )
            case 5:
            ISR_INSTALL_ISR2("SPI_H",Spi_Isr_F, DSPI_H_ISR_EOQF, 15, SPI_ARC_ISR_APP ); /*lint !e641 Converting enum to int*/
            break;
    #endif
    #if (SPI_USE_HW_UNIT_8 == STD_ON )
            case 5:
            ISR_INSTALL_ISR2("SPI_I",Spi_Isr_F, DSPI_I_ISR_EOQF, 15, SPI_ARC_ISR_APP ); /*lint !e641 Converting enum to int*/
            break;
    #endif
    #if (SPI_USE_HW_UNIT_9 == STD_ON )
            case 5:
            ISR_INSTALL_ISR2("SPI_J",Spi_Isr_F, DSPI_J_ISR_EOQF, 15, SPI_ARC_ISR_APP ); /*lint !e641 Converting enum to int*/
            break;
    #endif
            default:
                /*lint -e{1776} OTHER required operation */
    ...
    for (n = 0; n < numberOfChannels; n++) {
        ...
        if (chPtr->SpiDataWidth > 8) {
            ...
            for (uint16 i=0; i < txBufSize ; i++ ) {
                ...
                data = (txBuf != NULL_PTR) ? (uint32)tmpMisra : defaultValue;
...
void Spi_Hw_Init(const Spi_ConfigType *ConfigPtr) {
    ...
    /* Code from original Spi_Hw_Init(ConfigPtr) */
