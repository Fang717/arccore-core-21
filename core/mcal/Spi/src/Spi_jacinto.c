...
sint32 Spi_Hw_Tx(Spi_UnitType *uPtr)
{
...
#if defined(CFG_SPI_CS_TO_CLK_CALLBACK)
    Spi_CsToClkDelay();
#endif
    ...
    do {
        ...
        if( (chUnitCnt * bytesPerUnit) == uPtr->pData->chTxBytesCopied )
        {
               ...
               uPtr->txChIdxInJob++;
               if ( uPtr->jobPtr->SpiChannelAssignment[uPtr->txChIdxInJob] == NULL_PTR ) { /*lint !e9032 Comparison to NULL is ok */
                   uPtr->txChIdxInJob = 0;
...
sint32 Spi_Hw_Rx(Spi_UnitType *uPtr) {
    ...
    do
    {
        ...
        if( (chUnitCnt * bytesPerUnit) == uPtr->pData->chRxBytesCopied )
        {
            ...
            uPtr->rxChIdxInJob++;
            if ( uPtr->jobPtr->SpiChannelAssignment[uPtr->rxChIdxInJob] == NULL_PTR )	/*lint !e9032 Comparison to NULL is ok */
            {
                uPtr->rxChIdxInJob = 0;
                rv = SPIE_OK;
