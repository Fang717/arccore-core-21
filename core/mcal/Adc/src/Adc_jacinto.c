...
static void GroupConversionComplete(Adc_ArcControllerIdType ADCn){
    ...
    if( (Adc_Hw_GetConfigPTR(ADCn) != NULL_PTR)){
        // Check which group is busy, only one is allowed to be busy at a time in a hw unit
        for (group = 0; group <  Adc_Hw_GetConfigPTR(ADCn)->nbrOfGroups; group++){
            if( (Adc_Hw_GetConfigPTR(ADCn)->groupConfigPtr[group].status->groupStatus != ADC_IDLE)){
                groupPtr = &( Adc_Hw_GetConfigPTR(ADCn)->groupConfigPtr[group]);
                break;
            }
        }
    }
...
void Adc_Hw_Init(const Adc_ConfigType* ConfigPtr){
    ...
    Adc_GlobalType* Config_global = Adc_Hw_GetGlobalTypeAddr();

	memset(&Config_global, 0, sizeof(Config_global));

	Adc_Hw_GetGlobalTypeAddr()->config = (const Adc_ConfigType (*)[ADC_ARC_CTRL_CONFIG_CNT]) ConfigPtr; /*lint !e929 !e740 !e826 */
...
void Adc_Hw_StartGroupConversion (Adc_GroupType group){
    ...
    for (uint32 configId = 0; configId < ADC_ARC_CTRL_CONFIG_CNT; configId++) {
        if(Adc_Hw_GetConfigPTR(configId) != NULL_PTR){
        	if((Adc_Hw_GetConfigPTR(configId)->hwConfigPtr->hwUnitId == (group / ADC_NOF_GROUP_PER_CONTROLLER)) ){
                if( Adc_Hw_GetConfigPTR(configId)->nbrOfGroups > (group % ADC_NOF_GROUP_PER_CONTROLLER) ){
                    groupPtr =  &(Adc_Hw_GetConfigPTR(configId)->groupConfigPtr[group % ADC_NOF_GROUP_PER_CONTROLLER]);
                }
            }
        }
    }
 ...
 */
static inline Std_ReturnType validate (boolean exp,uint8 apiid,uint8 errid){
	Std_ReturnType validVal = E_OK;
	if(FALSE == exp){
		/*lint -e{506,774}  STANDARD DEV SELECT*/
		if(ADC_DEV_ERROR_DETECT == STD_ON){
			(void)Det_ReportError(ADC_MODULE_ID, 0, apiid, errid);
		}
		validVal = E_NOT_OK;
	}
	return validVal;
