...
void Adc_Hw_Init(const Adc_ConfigType* ConfigPtr){
	...
	Adc_Hw_GetGlobalTypeAddr()->config = ConfigPtr; /*lint !e929 !e740 !e826 */
...
static void GroupConversionComplete(Adc_ArcControllerIdType ADCn){
    ...
    for (configId = 0; configId < ADC_ARC_CTRL_CONFIG_CNT; configId++) {
        if(Adc_Hw_GetConfigPTR(configId) != NULL_PTR){
        	if( (Adc_Hw_GetConfigPTR(configId)->hwConfigPtr->hwUnitId == ADCn )){
                break;
            }
        }
    }
    ...
    if(Adc_Hw_GetConfigPTR(configId) != NULL_PTR){
        ...
        for (group = 0; group <  Adc_Hw_GetConfigPTR(configId)->nbrOfGroups; group++){
            if( Adc_Hw_GetConfigPTR(configId)->groupConfigPtr[group].status->groupStatus != ADC_IDLE){
                groupPtr = &( Adc_Hw_GetConfigPTR(configId)->groupConfigPtr[group]);
                break;
            }
        }
    }
