...
#if defined(CFG_MPC5777M) || defined(CFG_MPC5777C)||defined(CFG_MPC5748G)|| defined(CFG_MPC5746C) || defined(CFG_MPC5747C)
...
Std_ReturnType Fls_Erase(Fls_AddressType TargetAddress, Fls_LengthType Length) {
    ...
    /**
     * On MPC5634M, flash memory addresses are contiguous between all memory banks.
     * The memory bank1 is split into two memory arrays.
     * The two arrays share the same power source so both of the arrays cannot be accessed simultaneously.
     *
     * When multiple blocks that span between the two bank1 arrays are erased simultaneously,
     * only the blocks residing in the first array will be erased.
     * After that, the CPU shuts down reporting E_OS_PANIC error.
     *
     * To have it erase the whole requested memory,
     * we need to erase the blocks between two different memory arrays separately (one at a time).
     */
    Fls_AddressType MemoryArray2Start = 0x100000; // from the manual, page 233

    // if the requested blocks belong to memory array1 and memory array2
    if ((TargetAddress < MemoryArray2Start) && ((TargetAddress + Length) > MemoryArray2Start)) {
    	// erase the requested blocks from memory array1
        if (EE_OK != Flash_Erase(Fls_Global.config->FlsInfo, TargetAddress, MemoryArray2Start - TargetAddress, NULL )) {
            status = E_NOT_OK;
        }

        // wait for the first erase to finish
        uint32 flashStatus = EE_FIRST_TIME_INITIALIZATION;
        while (EE_OK != flashStatus) {
        	flashStatus = Flash_CheckStatus(Fls_Global.config->FlsInfo, TargetAddress, MemoryArray2Start - TargetAddress);
        	if (EE_ERROR_PE_OPT == flashStatus) {
        		status = E_NOT_OK;
        		break;
        	}
        }

        if (EE_OK == flashStatus) {
        	// erase the requested blocks from memory array2
			if (EE_OK != Flash_Erase(Fls_Global.config->FlsInfo, MemoryArray2Start, TargetAddress + Length - MemoryArray2Start, NULL )) {
				status = E_NOT_OK;
			}
        }
    } else {
    	// otherwise, erase the requested blocks as is
...
Std_ReturnType Fls_Write(Fls_AddressType TargetAddress,
    ...
    /** @req SWS_Fls_00027 */
    VALIDATE_W_RV( ((Length != 0uL) && ((((TargetAddress + Length) % FLASH_PAGE_SIZE) == 0uL) && (E_OK == checkValidAddress(TargetAddress + Length -1 )))),
...
void Fls_MainFunction(void) {
    ...
    static uint32 progressCntr = 0uL;
    uint32 flashStatus;
    sint32 result = 0L;
    uint8 eccError = 0u;
    ...
    if ( Fls_Global.jobResultType == MEMIF_JOB_PENDING) {
        ...
        case FLS_JOB_WRITE:
        {
            ...
            do {
                ...
                if (flashStatus == EE_OK) {
                    ...
                    if (flashStatus != EE_OK) {
                        ...
                        bytesLeftToBeWritten = (bytesWritten > bytesLeftToBeWritten) ? 0uL : (bytesLeftToBeWritten - bytesWritten);
                    }
            ...
            } while (bytesLeftToBeWritten > 0uL );
                            ...
                            Fls_LengthType Length)
{
    ...
    /** @req SWS_Fls_00098  */
    VALIDATE_W_RV( (Length != 0uL) && (E_OK == checkValidAddress(SourceAddress + Length -1)), FLS_READ_ID, FLS_E_PARAM_LENGTH, E_NOT_OK );
#if defined(CFG_MPC5777M)|| defined(CFG_MPC5777C)|| defined(CFG_MPC5748G)|| defined(CFG_MPC5746C) || defined(CFG_MPC5747C)
