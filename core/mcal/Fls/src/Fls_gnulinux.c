...
/** @tagSettings DEFAULT_ARCHITECTURE=GNULINUX */
...
static void write(int targetAddr, const char *srcDataPtr, int writeLen);
static void erase(int targetAddr, int length);
static void read(int sourceAddress, const char *targetAddressPtr, int length);
static int compare(int sourceAddress, const char *targetAddressPtr, int length);
...
static uint8 flsReadBatchJob(void) {
    ...
    if (jobLength != 0) {
        if (jobLength >= FLASH_PAGE_SIZE) {
            //reading a page
            read(jobAddress, &jobPtr[(count * FLASH_PAGE_SIZE)], FLASH_PAGE_SIZE);//read(sourceAddress, targetAddressPtr, length)

            jobLength = (jobLength) - (FLASH_PAGE_SIZE);
            jobAddress = jobAddress + (FLASH_PAGE_SIZE);
        }
        else {
            read(jobAddress, &jobPtr[(count * FLASH_PAGE_SIZE)], (size_t)jobLength);//read(sourceAddress, targetAddressPtr, length)
...
static uint8 flsWriteBatchJob(void) {
    ...
    if (jobLength != 0) {
        if (jobLength >= FLASH_PAGE_SIZE) {
        	...
        	write(jobAddress, &jobPtr[(count * FLASH_PAGE_SIZE)], jobLength); //write (targetAddr, srcDataPtr, writeLen
            jobLength = (jobLength) - (FLASH_PAGE_SIZE);
            jobAddress = jobAddress + (FLASH_PAGE_SIZE);
        }
        else {
...
static int init(void) {
	...
	/* 	Opening file if it exists or creating a file  */
	FLS_Ptr = fopen("Flash.bin", "rb+");
	if (FLS_Ptr == NULL) {
		printf("Unable to open file!\n");
		ret = E_NOT_OK;
	} else {

		/* 	Check if the size of the file is the same as the FLS_SIZE
			if flash size does not match FLS_SIZE the data is inconsistant
			rewritten
		*/

		fseek(FLS_Ptr, 0, SEEK_END);
		currentSize = ftell(FLS_Ptr);
		fseek(FLS_Ptr, 0, SEEK_SET);
		/* IMPROVEMENT: POXIX Implementation
		 * #include <sys/stat.h>
		 * struct stat st;
		 * stat(filename, &st);
		 * size = st.st_size;
		 */
		if (currentSize != FLS_TOTAL_SIZE) {
			for (counter = 0; counter < FLS_TOTAL_SIZE; counter++)
			fwrite(blank, sizeof(char), 1, FLS_Ptr);
			//IMPROVEMENT Throw DEM error?
		}
		fflush( FLS_Ptr );
	}
	return ret;
 ...
 * write:
 *
 * @param targetAddr: 	Address in the Flash were the data has to be written
 * @param srcDataPtr: 	pointer to the buffer to be written to the file
 * @param writeLen: 	The number of bytes that need to be written to the
 * 						from the buffer.
 */
static void write(int targetAddr, const char *srcDataPtr, int writeLen) {
	//IMPROVEMENT: Check if writeLen is less that valid data in srcDataPtr

	/* Goto begining of the file */
	fseek(FLS_Ptr, 0, SEEK_SET);
	/* Goto position to be written in the file of the file */
	fseek(FLS_Ptr, targetAddr, SEEK_SET);
	/* write */
	fwrite(srcDataPtr, sizeof(char), writeLen, FLS_Ptr);
	fflush( FLS_Ptr );
...
static void erase(int targetAddr, int length) {
	...
	fwrite(eraseData, sizeof(char), length, FLS_Ptr);
}
/**
 *
 * @param sourceAddress
...
static void read(int sourceAddress, const char *targetAddressPtr, int length){
	...
	fseek(FLS_Ptr, 0, SEEK_SET);
	/* Goto position to be written in the file of the file */
	fseek(FLS_Ptr, sourceAddress, SEEK_SET);
	fread (targetAddressPtr, 1, length, FLS_Ptr);
}
/**
 *
 * @param sourceAddress
...
static int compare(int sourceAddress,const char *targetAddressPtr, int length){
	...
	for (counter = 0; counter < length; counter++) {
		fread (readData, 1, 1, FLS_Ptr);
		if(targetAddressPtr[counter] != readData[0]){
			ret = E_NOT_OK;
		}
	}
	return ret;
        ...
        switch (pendingJob) {
            case FLS_JOB_COMPARE:
            {
                ...
                if (jobLength != 0) {
                    if (jobLength >= FLASH_PAGE_SIZE) {
                        //reading a page
                    	result = compare(jobAddress, &jobPtr[(count * FLASH_PAGE_SIZE)], FLASH_PAGE_SIZE);
                        jobLength = (jobLength) - (FLASH_PAGE_SIZE);
                        jobAddress = jobAddress + (FLASH_PAGE_SIZE);
                    }
                    else {
                        result = compare(jobAddress, &jobPtr[(count * FLASH_PAGE_SIZE)], jobLength);
                        jobLength = 0u;
                    }
                    count = count + 1u;
                }
