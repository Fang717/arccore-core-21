...
/** @tagSettings DEFAULT_ARCHITECTURE=PPC|MPC5607B|MPC5645S|MPC5748G|MPC5744P */
...
#define SWT SWT_A
    ...
    defined(CFG_MPC5777M) || defined(CFG_MPC5777C) || defined(CFG_MPC5644A) || defined(CFG_MPC5668) || defined(CFG_MPC563XM) || defined(CFG_MPC5747C) || defined(CFG_MPC5646B)
...
#elif defined(CFG_MPC5777M) || defined(CFG_MPC5777C)
    SWT.CR.R = 0xE000011BuL; /* Access for all cores */
    /*
     * Need to write these registers to clear softlock bit which is required to write into the timeout registers
     */
    /* Clear soft lock */
...
#	endif
    ...
    // If the watchdog is already ON prior to this call,
    // then we need to kick the watchdog so it resets the internal counter to the new set timeout.
    // Simply setting a new timeout will NOT reset the internal counter
    SWT.SR.R = 0x0000A602;
    SWT.SR.R = 0x0000B480;
...
#	endif
    ...
    /****
     * Unlocking possibly locked SOFT LOCK:
     * The soft lock is enabled by setting the SWT_CR[SLK] bit and is cleared by writing
     * the unlock sequence to the service register. The unlock sequence is a write of 0xC520 followed by a write of 0xD928
     ****/
    SWT.SR.R = 0x0000C520uL;
    SWT.SR.R = 0x0000D928uL;

    /****
     * Timeout can't be lower than 0x100 (see manual)
     * If user needs to set timeout by ms (Clocked by 128 kHz IRC): then timeout *= 128u
     * If user needs to set timeout by ms (Clocked by 16 MHz IRC): then timeout *= 16000u
     * ***/
    SWT.TO.R = (timeout <= 0x100uL)?  timeout * 128uL: timeout;

    /**** Access for the master ON,
     * FixedKeyServiceMode  0xA602, 0xB480 is used to service the watchdog,
     * ResetOnInvalidAccess ON ,
     * SoftLockOnRegisters ON,
     * 128kHz IRC,
     * freeze ON,
     * Enable ON
     * ****/
    ...
    /**** If the TIMEOUT watchdog is already ON prior to this call,
     * then we need to kick the watchdog so it resets the internal counter to the new set TIMEOUT.
     * Simply setting a new TIMEOUT will NOT reset the internal counter
     * ****/
...
#elif defined(CFG_MPC560X) || defined(CFG_MPC5668) || defined(CFG_MPC563XM) || defined(CFG_MPC5744P) || defined(CFG_MPC5645S) || defined(CFG_MPC5643L) || defined(CFG_SPC56XL70) || defined(CFG_MPC5748G) || defined(CFG_MPC5746C) || defined(CFG_MPC5747C) || defined(CFG_MPC5646B)
     SWT.SR.R = SOFTLOCK_WORD_1;     /* Write keys to clear soft lock bit */
     SWT.SR.R = SOFTLOCK_WORD_2;
     /*lint -e{970} -e{26} -e{10} -e{64} Lint does not seem handle typeof well */
...
void Wdg_Hw_Init (const Wdg_ConfigType* ConfigPtr)
{
    ...
    (void)Wdg_Hw_SetMode(ConfigPtr->Wdg_ModeConfig->Wdg_DefaultMode);
}
...
Std_ReturnType Wdg_Hw_SetMode (WdgIf_ModeType Mode)
{
    ...
    if (modeWdgConfig != NULL)
    {
        ...
        {
#if defined(CFG_MPC560X) || defined(CFG_MPC563XM) || defined(CFG_MPC5744P) || defined(CFG_MPC5645S) || defined(CFG_MPC5644A) || defined(CFG_MPC5777M) || defined(CFG_MPC5777C) || defined(CFG_MPC5748G) || defined(CFG_MPC5746C) || defined(CFG_MPC5747C)
...
#elif defined(CFG_MPC560X) || defined(CFG_MPC563XM) || defined(CFG_MPC5668) || defined(CFG_MPC5744P) || defined(CFG_MPC5645S) || defined(CFG_MPC5643L) || defined(CFG_SPC56XL70) || defined(CFG_MPC5644A) || defined(CFG_MPC5777C) || defined(CFG_MPC5777M)|| defined(CFG_MPC5748G) || defined(CFG_MPC5746C) || defined(CFG_MPC5747C) || defined(CFG_MPC5646B)
    SWT.SR.R = 0x0000A602uL;
    SWT.SR.R = 0x0000B480uL;
#elif defined(CFG_MPC5516)
    MCM.SWTSR.R = 0x55;
