...
/*lint -e9003 MISRA:FALSE_POSITIVE:could define variable 'Can_dcan_Unit' at block scope[MISRA 2012 Rule 8.9, advisory]*/
extern Can_dcan_UnitType Can_dcan_Unit[];
...
extern Can_dcan_GlobalType Can_dcan_Global;
#endif
...
static inline uint8 HwChannelToChannel(Can_Hw_CtrlType ch) {
    return Can_dcan_Global.channelMap[ch];
}
static inline Can_Hw_CtrlType ChannelToHwChannel(uint8 ch) {
    return Can_dcan_Global.channelToHwChannel[ch];
}
...
/* Old, soon deprecated */
void Can_dcan_Arc_DeInit(void);
void Can_dcan_Arc_Hw_DeInit(void);
...
void Can_dcan_Hw_MainFunction_Mode( void );
...
/**
 * @brief Hardware specific implementation of Can_dcan_CheckWakeup
 * @param[in] Controller - CAN controller id, for which the wakeup shall be checked
 * @return Can_ReturnType.
 * @retval CAN_OK: request accepted
 * @retval CAN_NOT_OK: request not accepted, a development error occurred
 */
Can_ReturnType Can_dcan_Hw_CheckWakeup( uint8 Controller );
...
Std_ReturnType Can_dcan_Hw_CheckBaudrate(uint8 Controller, const uint16 Baudrate);
//Std_ReturnType Can_dcan_Hw_ChangeBaudrate(uint8 Controller, const uint16 Baudrate);
 ...
 */
void Can_dcan_InstallInterruptHandlers(const Can_ControllerConfigType * ctrl);
 ...
 * @param[in] ctrl - Can controller config data pointer.
 */
void Can_dcan_RemoveInterruptHandlers(const Can_ControllerConfigType * ctrl);

/**
 * @brief Arccore specified function. Handle the interrupt of a certain controller.
 * @param[in] controllerNr - CAN controller id.
 */
void Can_dcan_InterruptHandler(CanControllerIdType controllerNr);
 ...
 */
const Can_dcan_HardwareObjectType * Can_FindHoh(Can_HwHandleType handle);
