...
ISR(Can1_dcan_Level0InterruptHandler);
ISR(Can2_dcan_Level0InterruptHandler);
ISR(Can3_dcan_Level0InterruptHandler);

/** @brief	Interrupt handler that service the CAN.
*			This handler handles the interrupt from DCAN1
 */
ISR(Can1_dcan_Level0InterruptHandler) {
    Can_dcan_InterruptHandler(DCAN1);
}
 ...
 */
ISR(Can2_dcan_Level0InterruptHandler) {
    Can_dcan_InterruptHandler(DCAN2);
}
 ...
 */
ISR(Can3_dcan_Level0InterruptHandler) {
    Can_dcan_InterruptHandler(DCAN3);
}
...
void Can_dcan_InstallInterruptHandlers(const Can_ControllerConfigType * ctrl) {
    if (ctrl->CanHwUnitId == DCAN1) {
        ISR_INSTALL_ISR2("DCAN1Level0", Can1_dcan_Level0InterruptHandler, CAN1_LVL0_INT, CAN_IRQ_PRIORITY, CAN_DCAN_ARC_ISR_APP );
    } else if (ctrl->CanHwUnitId == DCAN2) {
        ISR_INSTALL_ISR2("DCAN2Level0", Can2_dcan_Level0InterruptHandler, CAN2_LVL0_INT, CAN_IRQ_PRIORITY, CAN_DCAN_ARC_ISR_APP );
    /* @CODECOV:IDENTICAL_HARDWARE_INTERRUPTS: Covered achieved by Can1_dcan_Level0InterruptHandler and Can2_dcan_Level0InterruptHandler */
    } else if (ctrl->CanHwUnitId == DCAN3) {
        ISR_INSTALL_ISR2("DCAN3Level0", Can3_dcan_Level0InterruptHandler, CAN3_LVL0_INT, CAN_IRQ_PRIORITY, CAN_DCAN_ARC_ISR_APP );
    } else {
        /* Do nothing. */
    }
