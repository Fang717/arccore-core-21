 ...
 * @return Can_dcan_UnitType pointer.
 * @retval A controller handle, it is a structure.
 */
static Can_dcan_UnitType* const get_private_data(uint8 logController) {
    return &Can_dcan_Unit[logController];
}
 ...
 * @return Can_dcan_HardwareObjectType pointer.
 * @retval Hardware object handle.
 */
static const Can_dcan_HardwareObjectType * Can_FindHoh(Can_HwHandleType handle) {
    return &Can_dcan_Global.config->CanConfigSetPtr->ArcCanHardwareObjects[handle];
}
...
static void Can_HandleTxOk(CanControllerIdType controllerNr, uint8 MsgNr) {
    Can_dcan_UnitType* const canUnit = get_private_data((uint8) controllerNr);
    ...
    /** @req 4.1.2|4.3.0/SWS_Can_00016 The Can module shall call CanIf_dcan_TxConfirmation to indicate a successful transmission. It shall either called by the TX-interrupt service routine of the corresponding HW resource or inside the Can_dcan_MainFunction_Write in case of polling mode.*/
    CanIf_dcan_TxConfirmation(canUnit->transmittedPduIds[(MsgNr - 1)]);
}
...
static void Can_HandleBusOff(CanControllerIdType controllerNr) {
    ...
    status = Can_dcan_SetControllerMode(HwChannelToChannel(controllerNr), CAN_T_STOP); /*lint !e641 LINT:PERFORMANCE:convert enum to int */
...
/**
 * @brief Arccore specified function. Initialize the hardware register of a certain controller. It is called by Can_dcan_Hw_Init.
 * @param[in] controllerConfig - Pointer to controller configuration.
 */
static void Can_dcan_InitHardwareObjects(const Can_ControllerConfigType * controllerConfig) {
    uint32 	rx = 0x0uL;
    uint32 	tx = 0x0uL;
    ...
    uint8 	mbNr = 0x0u;
    uint8 mbN = 0x0u;
    const Can_dcan_HardwareObjectType* hoh = controllerConfig->Can_Arc_Hoh; //Configure the HOHs for this controller.
 ...
 */
Can_DriverStateType Can_dcan_Hw_GetDriverState(void) {
    return Can_dcan_Global.initRun;
}
...
/**
 * @brief  Return Can_dcan.hardware transmit handle.
 * @param[in] hth - Can_dcan.hardware unit handle Id.
 * @return Can_dcan_HardwareObjectType pointer.
 * @retval Can_dcan.hardware transmit handle.
 */
static inline const Can_dcan_HardwareObjectType * Can_FindTxHoh(Can_HwHandleType hth) {
    const Can_dcan_HardwareObjectType * hoh;
    hoh = &Can_dcan_Global.config->CanConfigSetPtr->ArcCanHardwareObjects[hth];
 ...
 * @param[in] regs - Pointer to Can_dcan.hardware register set.
...
static const Can_ControllerConfigType* getControllerConfig(uint8 logCh) {
    ...
    return &Can_dcan_Global.config->CanConfigSetPtr->CanController[logCh];
}

/**
 * @brief  Return a Can_dcan.hardware transmit handle.
...
static inline uint8 Can_FindControllerId(Can_HwHandleType handle) {
    const Can_dcan_HardwareObjectType * hoh = &Can_dcan_Global.config->CanConfigSetPtr->ArcCanHardwareObjects[handle];
    return hoh->ArcCanControllerId;
}

/** @brief Hardware specific implementation of Can_dcan_Init
 *  @param[in] Config - Pointer to configuration set
 */
void Can_dcan_Hw_Init(const Can_ConfigType *Config) {
    uint8 hwCh = 0u;

    /** @req 4.1.2|4.3.0/SWS_Can_00245 The function Can_dcan_Init shall initialize all CAN controllers according to their configuration.*/
    for (uint8 ch = 0; ch < CAN_ARC_LOG_UNIT_CNT; ch++) {
        ...
        /** @req 4.1.2|4.3.0/SWS_Can_00259 The function Can_dcan_Init shall set all CAN controllers in the state STOPPED.*//* Reset the controller. The device goes to Configuration Mode immediately. */
        Can_dcan_UnitType* const canUnit = get_private_data(ch);
        ...
        Can_dcan_InitHardwareObjects(controllerConfig);
        ...
        Can_dcan_InstallInterruptHandlers(controllerConfig);
#endif
    }
} /*IMPROVEMENT: PC-lint: Symbol 'Config' (line 377) not referenced will be solved when SSP-404 */

/**
 * @brief Hardware specific implementation of Can_dcan_SetControllerMode
 ...
 */
Can_ReturnType Can_dcan_Hw_SetControllerMode(uint8 Controller, Can_StateTransitionType Transition) {
    ...
    switch (Transition) {
        ...
        case CAN_T_WAKEUP:
            ...
            break;
        case CAN_T_STOP:
            /** @req 4.1.2|4.3.0/SWS_Can_00263 The function Can_dcan_SetControllerMode(CAN_CS_STOPPED) shall set the bits inside the CAN hardware such that the CAN controller stops participating on the network.*/
            /** @req 4.1.2|4.3.0/SWS_Can_00282 The function Can_dcan_SetControllerMode(CAN_CS_STOPPED) shall cancel pending messages.*/
 ...
 */
Can_Arc_ModeType Can_dcan_Hw_GetControllerMode( uint8 hwCh ) {
...
/** @brief Hardware specific implementation of Can_dcan_DeInit
 */
void Can_dcan_Hw_DeInit(void) {
    Can_dcan_UnitType* canUnit;
    ...
    for (uint8 ch = 0; ch < CAN_ARC_CTRL_CONFIG_CNT; ch++) {
        ...
        Can_dcan_RemoveInterruptHandlers(getControllerConfig(ch));
    }
}

/** @brief Hardware specific implementation of Can_dcan_DisableControllerInterrupts. Does not perform any other logic like changing the disable interrupt count.
 *  @param[in] Controller - CAN controller id, for which the interrupts shall be disabled.
 */
void Can_dcan_Hw_DisableControllerInterrupts(uint8 Controller) {
...
/** @brief Hardware specific implementation of Can_dcan_EnableControllerInterrupts
 ...
 */
void Can_dcan_Hw_EnableControllerInterrupts(uint8 Controller) {
    Can_RegisterType *canCtrlRegs = CanRegs[Controller];
...
/**
 * @brief Hardware specific implementation of Can_dcan_Write
 ...
 */
Can_ReturnType Can_dcan_Hw_Write(Can_HwHandleType Hth, const Can_PduType *PduInfo) {
    Can_ReturnType ret = CAN_OK;
    Std_ReturnType validate_rv = E_OK;
    uint32 ArbRegValue;
    uint8 DataByteIndex;
    uint8 ControllerId = Can_FindControllerId(Hth);
    const Can_dcan_HardwareObjectType *hoh = Can_FindTxHoh(Hth);
    ...
    if (E_OK == validate_rv) {
        ...
        Can_dcan_UnitType* const canUnit = get_private_data(controllerConfig->CanHwUnitId); /*lint !e641 LINT:PERFORMANCE:convert enum to int */
        Can_RegisterType *canCtrlRegs = CanRegs[controllerConfig->CanHwUnitId];
        ...
        if (hoh->ArcMailboxMask != (hoh->ArcMailboxMask & canUnit->mxBoxBusyMask)) {
            ...
            /** @req 4.1.2|4.3.0/SWS_Can_00276 The function Can_dcan_Write shall store the swPduHandle that is given inside the parameter PduInfo until the Can module calls the CanIf_dcan_TxConfirmation for this request where the swPduHandle is given as parameter.*/
            ...
            /** @req 4.1.2|4.3.0/SWS_Can_00214 The function Can_dcan_Write shall return CAN_BUSY if a preemptive call of Can_dcan_Write has been issued, that could not be handled reentrant (i.e. a call with the same HTH).*/
            ret = CAN_BUSY;
        }
    }
    else {
 ...
 */
void Can_dcan_HandleRxOk(uint8 MsgObjectNr, const Can_ControllerConfigType * controllerConfig, const Can_dcan_HardwareObjectType * hrh) {
    uint32 MsgId;
    uint8 MsgDlc;
    uint8 DataByteIndex;
    uint8 *SduPtr;
    ...
    do {
        ...
        /* @CODECOV:REDUNDANT_PARAMETER_VALIDATION: Can_dcan_HandleRxOk is called from interrupt handler always when there is a message waiting */
        ...
        if ((mc & (1uL << DCAN_MC_NEWDAT)) == (1uL << DCAN_MC_NEWDAT)) {
            ...
            /* @CODECOV:CONFIG_LIMITATION: Cannot be tested in loopback mode, because Can_dcan_Write validates if Dlc <= 8 */
            if (MsgDlc > 8) {
                MsgDlc = 8;
            }
            ...
            /** @req 4.1.2|4.3.0/SWS_Can_00279 On L-PDU reception, the Can module shall call the RX indication callback function CanIf_dcan_RxIndication with ID, Hoh, abstract CanIf ControllerId in parameter Mailbox, and the Data Length and pointer to the L-SDU buffer in parameter PduInfoPtr.*/
            /** @req 4.1.2|4.3.0/SWS_Can_00396 The RX-interrupt service routine of the corresponding HW resource or the function Can_dcan_MainFunction_Read in case of polling mode shall call the callback function CanIf_dcan_RxIndication.*/
            CanIf_dcan_RxIndication((hrh->CanObjectId) & 0xFFuL, MsgId, MsgDlc, SduPtr);
 ...
 * @param[in] controllerNr - CAN controller id.
 */
void Can_dcan_InterruptHandler(CanControllerIdType hwCh) {
    ...
    while ((ir = canRegs->INT) != 0) {
        ...
        if ((ir & CAN_REG_IR_INT0ID_MASK) == CAN_REG_IR_BUSEVENT) {
            ...
            if ((sr & (1uL << CAN_REG_ES_TXOK_BIT)) == (1uL << CAN_REG_ES_TXOK_BIT)) {
                ...
                CanIf_dcan_ControllerBusOff(ch); /*lint !e641 LINT:PERFORMANCE:convert enum to int */
            ...
            const Can_dcan_HardwareObjectType * hoh = Can_FindHoh(hohHandle);

            if (hoh->CanObjectType == CAN_OBJECT_TYPE_RECEIVE) {
                Can_dcan_HandleRxOk((uint8) (ir & CAN_REG_IR_INT0ID_MSG_MASK), controllerConfig, hoh);
            }
            else {
...
Std_ReturnType Can_dcan_Hw_ChangeBaudrate(uint8 Controller, const Can_ControllerBaudrateConfigType *baudratePtr ) {
    /* NOT SUPPORTED */
    return E_NOT_OK;
}
