...
Std_ReturnType Lin_SendFrame(  uint8 Channel,  Lin_PduType* PduInfoPtr )
{
    ...
    uint16 timeOutValue = (uint16)( ( ( 10 * PduInfoPtr->Dl + 45 ) * 14 ) / 10);
    ...
    if (PduInfoPtr->Drc == LIN_MASTER_RESPONSE)
    {
        ...
        if (PduInfoPtr->Dl > 0){
            ...
            TxSize[Channel] = PduInfoPtr->Dl;
            /* @req SWS_Lin_00053 */
            /* @req SWS_Lin_00025 */
            /* @req SWS_Lin_00026 */
            /* @req SWS_Lin_00096 */
            memcpy(TxCurrPtr[Channel],PduInfoPtr->SduPtr,PduInfoPtr->Dl);
        }
        esciHw->LTR.R=tmpLtr.R; /* write to transmit reg */
    }
