...
#define IS_VALID_CHANNEL(_x) ( ((_x) < ICU_NUMBER_OF_CHANNELS) && \
                               ...
                               (((GetPhysicalChannel(_x)) <= 23) && ((GetPhysicalChannel(_x)) >= 8))))
#elif defined(CFG_MPC5748G)/*CFG_MPC5748G*/
#define IS_VALID_CHANNEL(_x) ( ((_x) < ICU_NUMBER_OF_CHANNELS) && \
                               ((GetPhysicalChannel(_x)) <= (ICU_MAX_CHANNEL-1)) )
#else  /* CFG_MPC560XB and CFG_MPC5746C and CFG_MPC5646B */
#define IS_VALID_CHANNEL(_x) ( ((_x) < ICU_NUMBER_OF_CHANNELS) && \
                               ((GetPhysicalChannel(_x)) <= (ICU_MAX_CHANNEL-1)) )
#endif
...
static void configureChannel(Icu_ChannelType channelId, const Icu_ChannelConfigurationType* channelConfig){
...
#if defined(CFG_MPC5645S) || defined (CFG_MPC560XB) || defined(CFG_MPC5606S) || defined(CFG_MPC5746C) || defined(CFG_MPC5646B)
    if(channel <= (ICU_NUMBER_OF_EACH_EMIOS-1)) {
		...
		if(channel <= (ICU_NUMBER_OF_EACH_EMIOS-1)) {
            emiosHw = &EMIOS_0;
        } else if(channel <= ((ICU_NUMBER_OF_EACH_EMIOS*2)-1)) {
            emiosHw = &EMIOS_1;
        } else{
            emiosHw = &EMIOS_2;
        }
...
#endif
#if defined(CFG_MPC560XB) || defined(CFG_MPC5645S) || defined(CFG_MPC5606S) || defined(CFG_MPC5746C) || defined(CFG_MPC5646B) || defined(CFG_MPC5748G)
    emiosHw->CH[channel].CCR.B.BSL = channelConfig->clksrc; /* Bus select */
    emiosHw->CH[channel].CCR.B.UCPRE = channelConfig->prescaler;
#else
    emiosHw->CH[channel].CCR.B.BSL = 0u; /* Bus select */
    emiosHw->CH[channel].CCR.B.UCPRE = 1u;
#endif
#if defined(CFG_MPC5746C)|| defined(CFG_MPC5748G)
    emiosHw->CH[channel].CCR.B.UCPREN = 1u;
#else
    emiosHw->CH[channel].CCR.B.UCPEN = 1u;
#endif
    emiosHw->CH[channel].CCR.B.IF = 1u;
    emiosHw->CH[channel].CCR.B.FREN = 1u;
    emiosHw->CH[channel].CCR.B.MODE = 0u; /* Reset by setting GPIO */

    if( channelConfig->default_start_edge == ICU_BOTH_EDGES ) {
        emiosHw->CH[channel].CCR.B.EDSEL = 1u; /* Both edges triggering */
    }
    else {
        emiosHw->CH[channel].CCR.B.EDSEL = 0u; /* Single edge triggering */
        if( channelConfig->default_start_edge == ICU_RISING_EDGE ) {
            emiosHw->CH[channel].CCR.B.EDPOL = 1u; /* Trigger on a rising edge */
        }
        else if( channelConfig->default_start_edge == ICU_FALLING_EDGE ) {
            emiosHw->CH[channel].CCR.B.EDPOL = 0u; /* Trigger on a falling edge */
        }
        else {
            /* MISRA */;
        }
...
{
    /* Install ISR */
#if defined(CFG_MPC560XB) || defined(CFG_MPC5746C) || defined(CFG_MPC5646B)
    ...
    case 64:
    case 65:
        ISR_INSTALL_ISR2("IcuIsr", Icu_Isr, (int16_t)(EMIOS_2_GFR_F0_F1),ICU_ISR_PRIORITY, 0);
        break;
    case 66:
    case 67:
         ISR_INSTALL_ISR2("IcuIsr", Icu_Isr, (int16_t)(EMIOS_2_GFR_F2_F3),ICU_ISR_PRIORITY, 0);
        break;
    case 68:
    case 69:
        ISR_INSTALL_ISR2("IcuIsr", Icu_Isr, (int16_t)(EMIOS_2_GFR_F4_F5),ICU_ISR_PRIORITY, 0);
        break;
    case 70:
    case 71:
        ISR_INSTALL_ISR2("IcuIsr", Icu_Isr, (int16_t)(EMIOS_2_GFR_F6_F7),ICU_ISR_PRIORITY, 0);
         break;
    case 72:
    case 73:
         ISR_INSTALL_ISR2("IcuIsr", Icu_Isr, (int16_t)(EMIOS_2_GFR_F8_F9),ICU_ISR_PRIORITY, 0);
         break;
    case 74:
    case 75:
         ISR_INSTALL_ISR2("IcuIsr", Icu_Isr, (int16_t)(EMIOS_2_GFR_F10_F11),ICU_ISR_PRIORITY, 0);
         break;
    case 76:
    case 77:
         ISR_INSTALL_ISR2("IcuIsr", Icu_Isr, (int16_t)(EMIOS_2_GFR_F12_F13),ICU_ISR_PRIORITY, 0);
         break;
    case 78:
    case 79:
         ISR_INSTALL_ISR2("IcuIsr", Icu_Isr, (int16_t)(EMIOS_2_GFR_F14_F15),ICU_ISR_PRIORITY, 0);
         break;
    case 80:
    case 81:
        ISR_INSTALL_ISR2("IcuIsr", Icu_Isr, (int16_t)(EMIOS_2_GFR_F16_F17),ICU_ISR_PRIORITY, 0);
        break;
    case 82:
    case 83:
        ISR_INSTALL_ISR2("IcuIsr", Icu_Isr, (int16_t)(EMIOS_2_GFR_F18_F19),ICU_ISR_PRIORITY, 0);
        break;
    case 84:
    case 85:
        ISR_INSTALL_ISR2("IcuIsr", Icu_Isr, (int16_t)(EMIOS_2_GFR_F20_F21),ICU_ISR_PRIORITY, 0);
        break;
    case 86:
    case 87:
        ISR_INSTALL_ISR2("IcuIsr", Icu_Isr, (int16_t)(EMIOS_2_GFR_F22_F23),ICU_ISR_PRIORITY, 0);
        break;
    case 88:
    case 89:
        ISR_INSTALL_ISR2("IcuIsr", Icu_Isr, (int16_t)(EMIOS_2_GFR_F24_F25),ICU_ISR_PRIORITY, 0);
        break;
    case 90:
    case 91:
        ISR_INSTALL_ISR2("IcuIsr", Icu_Isr, (int16_t)(EMIOS_2_GFR_F26_F27),ICU_ISR_PRIORITY, 0);
        break;
    case 92:
    case 93:
        ISR_INSTALL_ISR2("IcuIsr", Icu_Isr, (int16_t)(EMIOS_2_GFR_F28_F29),ICU_ISR_PRIORITY, 0);
        break;
    case 94:
    case 95:
        ISR_INSTALL_ISR2("IcuIsr", Icu_Isr, (int16_t)(EMIOS_2_GFR_F30_F31),ICU_ISR_PRIORITY, 0);
        break;
...
void Icu_Init(const Icu_ConfigType* ConfigPtr) {
    ...
    EMIOS_0.MCR.B.GTBE = 1u;
    EMIOS_1.MCR.B.GTBE = 1u;

    /* Enable clock */
    EMIOS_0.MCR.B.MDIS = 0u;
    EMIOS_1.MCR.B.MDIS = 0u;
#elif defined(CFG_MPC5748G)
    /* Stop channels when in debug mode */
    EMIOS_0.MCR.B.FRZ = ICU_FREEZE_ENABLE;
    EMIOS_1.MCR.B.FRZ = ICU_FREEZE_ENABLE;
    EMIOS_2.MCR.B.FRZ = ICU_FREEZE_ENABLE;
    /* Use global time base */
    EMIOS_0.MCR.B.GTBE = 1u;
    EMIOS_1.MCR.B.GTBE = 1u;
    EMIOS_2.MCR.B.GTBE = 1u;
    /* Enable clock */
    EMIOS_0.MCR.B.MDIS = 0u;
    EMIOS_1.MCR.B.MDIS = 0u;
    EMIOS_2.MCR.B.MDIS = 0u;
#endif
    Icu_ChannelType channel_iterator;

    for (channel_iterator = 0u; channel_iterator < ICU_NUMBER_OF_CHANNELS; channel_iterator++) {
...
#elif defined(CFG_MPC5748G)
        if(physical_channel <= (ICU_NUMBER_OF_EACH_EMIOS-1)) {
           hwUnit = 0u;
		} else if(physical_channel <= ((ICU_NUMBER_OF_EACH_EMIOS*2)-1)) {
           hwUnit = 1u;
		} else{
           hwUnit = 2u;
		}
...
static void DeInitChannel(Icu_ChannelType Channel) {
...
#if defined(CFG_MPC5645S) || defined(CFG_MPC560XB) || defined(CFG_MPC5606S)|| defined(CFG_MPC5746C) || defined(CFG_MPC5646B)
    emiosHw = (emios_ch <= (ICU_NUMBER_OF_EACH_EMIOS-1))? &EMIOS_0 : &EMIOS_1;
    emios_ch %= ICU_NUMBER_OF_EACH_EMIOS;
#elif defined(CFG_MPC5748G)
		if(emios_ch <= (ICU_NUMBER_OF_EACH_EMIOS-1)) {
           emiosHw = &EMIOS_0;
        } else if(emios_ch <= ((ICU_NUMBER_OF_EACH_EMIOS*2)-1)) {
           emiosHw = &EMIOS_1;
		} else{
           emiosHw = &EMIOS_2;
		}
    ...
    emiosHw->CH[emios_ch].CADR.R = 0uL;
    emiosHw->CH[emios_ch].CCR.B.MODE = 0u; /* Set to GPIO Mode */
    emiosHw->CH[emios_ch].CCR.B.EDSEL = 0u; /* Single edge triggering */
    emiosHw->CH[emios_ch].CCR.B.EDPOL = 0u; /* Trigger on a falling edge */
...
#if defined(CFG_MPC5645S) || defined(CFG_MPC560XB) || defined(CFG_MPC5606S)|| defined(CFG_MPC5746C) || defined(CFG_MPC5646B) || defined(CFG_MPC5748G)
    emiosHw->CH[emios_ch].CCR.B.MODE = EMIOS_UC_MODE_SAIC; /* Single Action Input Capture */

    if( Activation == ICU_BOTH_EDGES ) {
        emiosHw->CH[emios_ch].CCR.B.EDSEL = 1u; /* Both edges triggering */
    }
    else {
        emiosHw->CH[emios_ch].CCR.B.EDSEL = 0u; /* Single edge triggering */
        if( Activation == ICU_RISING_EDGE ) {
            emiosHw->CH[emios_ch].CCR.B.EDPOL = 1u; /* Trigger on a rising edge */
        }
    ...
    if (IcuChannelRuntimeStruct[Channel].NextTimeStampIndex >= (IcuChannelRuntimeStruct[Channel].TimeStampBufferSize))
    {
        switch (IcuConfigPtr->Channels[Channel].buffer_type)
        {
           case ICU_LINEAR_BUFFER:
                ...
                emiosHw->CH[emios_ch].CCR.B.FEN = 0u;
...
static void SignalMeasurementISR(Icu_ChannelType Channel)
{
    ...
    volatile struct EMIOS_tag *emiosHw;
#if defined(CFG_MPC5645S) || defined(CFG_MPC560XB) || defined(CFG_MPC5746C) || defined(CFG_MPC5646B)
    emiosHw = (emios_ch <= (ICU_NUMBER_OF_EACH_EMIOS-1))? &EMIOS_0 : &EMIOS_1;
    emios_ch %= ICU_NUMBER_OF_EACH_EMIOS;
#elif defined(CFG_MPC5748G)
        if(emios_ch <= (ICU_NUMBER_OF_EACH_EMIOS-1)) {
    ...
    if ((emiosHw->CH[emios_ch].CSR.B.OVFL == 1u) ||
            (endTime < startTime))
    {
        endTime += IcuChannelRuntimeStruct[Channel].Max_Counter;

        /* Clear overflow flag */
        emiosHw->CH[emios_ch].CSR.B.OVFL = 1u;
    }
...
static void Init_Signal_Measurement(Icu_ChannelType Channel)
{
    ...
    IcuChannelRuntimeStruct[Channel].PreviousEndTime = 0u;
...
static void Init_Timestamping(Icu_ChannelType Channel, Icu_ValueType* StartPtr, uint16 Size, uint16 NotifyInterval)
{
    ...
    IcuChannelRuntimeStruct[Channel].NextTimeStampIndex = 0u;
    IcuChannelRuntimeStruct[Channel].NotifyInterval = NotifyInterval;
    IcuChannelRuntimeStruct[Channel].NotificationCounter = 0u;
}
...
static void Clear_Timestamping(Icu_ChannelType Channel)
{
    ...
    IcuChannelRuntimeStruct[Channel].TimeStampBufferSize = 0u;
    IcuChannelRuntimeStruct[Channel].NextTimeStampIndex = 0u;
    IcuChannelRuntimeStruct[Channel].NotifyInterval = 0u;
    IcuChannelRuntimeStruct[Channel].NotificationCounter = 0u;
}
...
void Icu_StartTimestamp( Icu_ChannelType Channel, Icu_ValueType* BufferPtr, uint16 BufferSize, uint16 NotifyInterval ){
    ...
    emiosHw->CH[emios_ch].CSR.B.FLAG = 1u;
    emiosHw->CH[emios_ch].CCR.B.FEN = 1u;

    /* Reset counter */
    emiosHw->CH[emios_ch].CCR.B.MODE = 0u; /* Reset by setting GPIO */
    emiosHw->CH[emios_ch].CCR.B.MODE = EMIOS_UC_MODE_SAIC; /* Single Action Input Capture */
...
#   if defined(CFG_MPC5645S) || defined(CFG_MPC560XB) || defined(CFG_MPC5606S) || defined(CFG_MPC5746C) || defined(CFG_MPC5646B)
    ...
    uint32_t flagmask_0 = (uint32_t) EMIOS_0.GFR.R;
    uint32_t flagmask_1 = (uint32_t) EMIOS_1.GFR.R;
    uint32_t flagmask_2 = (uint32_t) EMIOS_2.GFR.R;
#   else   
    flagmask = (uint32_t) EMIOS.GFR.R; 
#   endif
     
    for (Icu_ChannelType channel_iterator = 0; channel_iterator < ICU_NUMBER_OF_CHANNELS; channel_iterator++) {

        emios_ch = IcuConfigPtr->Channels[channel_iterator].physical_channel;
...
#   if defined(CFG_MPC5645S) || defined(CFG_MPC560XB) || defined(CFG_MPC5606S) || defined(CFG_MPC5746C) || defined(CFG_MPC5646B)
        ...
        /* better with safety checking for range of channels */
        emios_ch %= ICU_NUMBER_OF_EACH_EMIOS;
#elif defined(CFG_MPC5748G)
...
#   endif
        ...
        if ((emiosHw->CH[emios_ch].CCR.B.FEN != 0u) && ((flagmask & (1UL << emios_ch)) != 0uL)) {
            
            switch(IcuConfigPtr->Channels[channel_iterator].measurement_mode)    
            {
#if (ICU_TIMESTAMP_API == STD_ON)
