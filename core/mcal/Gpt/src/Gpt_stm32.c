...
#define MAXIMUM_TICK_VALUE 65535ul
...
void Gpt_StartTimer(Gpt_ChannelType channel, Gpt_ValueType period_ticks)
{
    ...
    VALIDATE( ((period_ticks != 0UL) && (period_ticks <= MAXIMUM_TICK_VALUE )), GPT_STARTTIMER_SERVICE_ID, GPT_E_PARAM_VALUE);
    ...
    {
        Gpt_ValueType *tlval = (uint32 *)&(TimAddr[channel]->ARR);
        Gpt_ValueType *tval = (uint32 *)&(TimAddr[channel]->CNT);
