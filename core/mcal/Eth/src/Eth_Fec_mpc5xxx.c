...
static inline void addUnicastAdrsTablEntry(uint8 ethHwUnit,const uint8* pMacAdrs){
    ...
    if (crc >= 32) {
        pHwPtr->IAUR.B.IADDR1 |= (1uL << (crc - 32));
        pHwPtr->IALR.B.IADDR2 = 0; //added for mpc5777c
    } else {
        pHwPtr->IAUR.B.IADDR1 = 0; //added for mpc5777c
        pHwPtr->IALR.B.IADDR2 |= (1uL << crc);
    }
}
...
static Std_ReturnType configureController( const Eth_ControllerConfigType *pCfgPtr) {
...
#if (ETH_USE_HW_UNIT_0 == STD_ON )
            INSTALL_TX_HANDLER(0, FEC_EIR_TXF);/*lint !e9036  No harmness*/
#endif
        }
    }
    if(TRUE == pCfgPtr->EthCtrlEnableTxInterrupt) {
        if (pCfgPtr->EthHwUnit == HWUNIT_0){
#if (ETH_USE_HW_UNIT_0 == STD_ON )
            INSTALL_RX_HANDLER(0, FEC_EIR_RXF);/*lint !e9036  No harmness*/
#endif
        }
    }
#if defined(CFG_ETH_DEBUG)
    if (pCfgPtr->EthHwUnit == HWUNIT_0){
#if (ETH_USE_HW_UNIT_0 == STD_ON )
        INSTALL_ERR_HANDLER(0, FEC_EIR_ERROR);/*lint !e9036   No harmness*/
#endif
    }
#endif
#endif
    ...
    pHwPtr->OPD.B.PAUSE_DUR = 0x01; /* Pause Duration for TX Pause frames */
...
Std_ReturnType Eth_Hw_SetControllerMode(
    ...
    VALIDATE_HW_RV(CtrlCfgPtr->EthHwUnit, ETH_GLOBAL_ID, ETH_E_CONFIG_INVALID, E_NOT_OK);
    Std_ReturnType ret = E_OK;
    switch (CtrlMode) {
        case ETH_MODE_ACTIVE:
            if (E_OK != setCommunicationMode(ETH_MODE_ACTIVE, CtrlIdx)) {
                ret = E_NOT_OK;
            }
            break;
        case ETH_MODE_DOWN:
