...
static void insertEthHeader(const Eth_ControllerConfigType *CtrlCfgPtr,
    ...
    /* Src MAC */
    for (i = 0; i < ETH_MAC_ADDR_SIZE; i++) {
        dataPtr[ETH_MAC_SRC_ADDR_START_INDEX + i] =
                CtrlCfgPtr->EthCtrlPhyAddress[i];
    }
    /* Ethernet type */
    ...
    for (Eth_BufIdxType bufIndex = 0;
        ...
        if ( FALSE
            ...
            if (*LenBytePtr
                ...
                ret = BUFREQ_E_OVFL;
                *LenBytePtr = (uint16) CtrlCfgPtr->EthCtrlTxBufLenByte
                        - ETH_HEADER_SIZE;
            }
            else {
                EthCtrlUnit[CtrlCfgPtr->EthCtrlIdx].EthTxBufferStatus[bufIndex].locked =
    ...
    do {
        ...
        if (ETH_NOT_RECEIVED != rxStatus) {
            ...
            for (nextRxDescIndex = 0;
                    ...
                    nextRxDescIndex++) {
                if ((pEthRxDescriptor[nextRxDescIndex].Address
                        & ETH_GEM_RXBUF_USED) != 0) {
                    if ( ETH_GEM_RXBUF_FRAME_SINGLE
                        ...
                        for (uint8 i = 0;
                                (i < ETH_MAC_ADDR_SIZE) && isBroadcast; i++) {
