...
static void configureMDIO(uint8 hwUnit){
    ...
    /* calculate the MDIO speed with manual override.*/
    /* N.B. MSCR.B.MII_SPEED controls the frequency of the MII management interface clock (MDC) relative to the internal module clock,
     * i.e. it is not the actual MII(Media Independent Interface) speed.
     */
    #if defined(ETH_MDIO_SPEED)
    pHwPtr->MSCR.B.MII_SPEED = (ETH_MDIO_SPEED);
    #else
...
void Eth_RxIrqHdlr(uint8 HwUnit) {
    ...
    pHwPtr->EIR.R = ievent; // Clear before handling
