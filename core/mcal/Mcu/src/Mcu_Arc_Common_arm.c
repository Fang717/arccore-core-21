...
{
	/*lint --e{904} MISRA :ARGUMENT CHECK::[MISRA 2012 Rule 15.5, required] */
	uint32 i = 0;
...
/* As this is test only function we should exclude it from coverage report
 */
...
void Mcu_Arc_PrintInfo( void )
{
    ...
    /*lint --e{705} LINT:OTHER:suppress "Argument no. 2 nominally inconsistent with format (int vs. uint32)" */
    while( cpu_info_list[i].name != NULL  ) {
        printf( "CPU: %s\n"
