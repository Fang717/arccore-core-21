...
/** @tagSettings DEFAULT_ARCHITECTURE=PPC|MPC5607B|MPC5645S|MPC5646B|MPC5777M|MPC5777C */
...
#if defined(CFG_MPC560X) || defined(CFG_MPC5645S) || defined (CFG_MPC5643L) || defined(CFG_SPC56XL70) || defined(CFG_MPC5646B)
    uint16 rawValueDES;
    uint16 rawValueFES;
...
const core_info_t core_info_list[] = {
...
#elif defined(CFG_MPC5645S)
    {
        .name = "MPC5645S",
        .pvr = CORE_PVR_E200Z4D_1,
        .info = "Max Freq: 125MHz, Cache: 4K Instruction",
    },
#elif defined(CFG_MPC5646B)
    {
        .name = "MPC5646B",
        .pvr = CORE_PVR_E200Z4D_1,
        .info = "Max Freq: 120MHz, Cache: 4K Instruction",
    },
#endif
};
/*lint -restore */
...
Std_ReturnType Mcu_Hw_InitClock(const Mcu_ClockSettingConfigType *clockSettingsPtr)
{
    ...
    FMPLL.SYNCR.B.DEPTH	= 0u;
    FMPLL.SYNCR.B.LOLRE	= 0u;
    FMPLL.SYNCR.B.LOLIRQ = 0u;
...
#if !defined(CFG_SIMULATOR)
#if defined(CFG_MPC560XB) || defined(CFG_MPC5646B)
        /*lint -e{923}  Ok, this cast is in Freescale header file */
...
#define ME MC_ME
#endif
    ME.MCTL.R = 0x00005AF0uL;   /* Reset, 0x5AF0 is control key to enable writing to register */
    ME.MCTL.R = 0x0000A50FuL;   /* Reset, 0xA50F is inverted control key to enable writing to register */

    while(ME.GS.B.S_MTRANS != 0u) {}
    while(ME.GS.B.S_CURRENTMODE != 0u) {}
#else
    SIU.SRCR.B.SSR = 1u;
#endif
    /*lint -restore */
...
uint32 Mcu_Arc_GetPeripheralClock(Mcu_Arc_PeriperalClock_t type) {
	 ...
	 * 
	 * FEC -> max freq. 80MHz
	 * FLASH memory BIU -> max freq. 80MHz
	 * note: e200e0h max freq. 80MHz	
     */

    switch (type)
    {
		/* peripheral set 1 -> max freq. 32MHz */
		case PERIPHERAL_CLOCK_IIC_A:
		...
		case PERIPHERAL_CLOCK_LIN_C:
		case PERIPHERAL_CLOCK_LIN_D:
		case PERIPHERAL_CLOCK_LIN_E:
		case PERIPHERAL_CLOCK_LIN_F:
		case PERIPHERAL_CLOCK_LIN_G:
		case PERIPHERAL_CLOCK_LIN_H:
		case PERIPHERAL_CLOCK_LIN_I:
		case PERIPHERAL_CLOCK_LIN_J:
		
			if( CGM.SC_DC0.B.DE0 ) {
                prescaler = CGM.SC_DC0.B.DIV0 + 1u;
				}
			
			switch( CGM.SC_SS.B.SELSTAT ) {
				 case 0:
					 /* FIRC */
					 sysClock = 16000000uL;
					 break;
				 ...
				 case 2:
					 /* EXTAL */
					 sysClock = Mcu_Global.config->McuClockSettingConfig[Mcu_Global.clockSetting].McuClockReferencePointFrequency;
					 break;
				 ...
				 case 4:
					...
					uint32  extal = Mcu_Global.config->McuClockSettingConfig[Mcu_Global.clockSetting].McuClockReferencePointFrequency;
					sysClock = CALC_SYSTEM_CLOCK(extal,emfd,eprediv,erfd);             
					 break;
				 ...
				 default: /* unknown/reserved value */
					 ASSERT(0);
					 break;
			}
		...
		/* peripheral set 2 -> max freq. 64MHz */		
		case PERIPHERAL_CLOCK_DSPI_A:
		case PERIPHERAL_CLOCK_DSPI_B:
		case PERIPHERAL_CLOCK_DSPI_C:
		case PERIPHERAL_CLOCK_DSPI_D:
		...
		case PERIPHERAL_CLOCK_DSPI_G:
		case PERIPHERAL_CLOCK_DSPI_H:
		...
		case PERIPHERAL_CLOCK_FLEXCAN_F:
		    ...
		    if( CGM.SC_DC1.B.DE1 ) {
                prescaler = CGM.SC_DC1.B.DIV1 + 1u;
            }
		...
		/* peripheral set 3 -> max freq. 64MHz */	
		case PERIPHERAL_CLOCK_ADC_0:
		case PERIPHERAL_CLOCK_ADC_1:
		case PERIPHERAL_CLOCK_CTU:
		case PERIPHERAL_CLOCK_EMIOS_0:
		case PERIPHERAL_CLOCK_EMIOS_1:
		
			if( CGM.SC_DC2.B.DE2 ) {
                prescaler = CGM.SC_DC2.B.DIV2 + 1u;
            }
		...
		/*case PERIPHERAL_CLOCK_E200Z0:
			prescaler = CGM.Z0_DCR.B.DIV  + 1uL;
			break;
		*/	
			
		case PERIPHERAL_CLOCK_CFLASH_0:
			prescaler = CGM.FLASH_DCR.B.DIV  + 1uL;
			break;
			
		case PERIPHERAL_CLOCK_DFLASH:
			prescaler = CGM.FLASH_DCR.B.DIV  + 1uL;
			break;
			
		case PERIPHERAL_CLOCK_CFLASH_1:
			prescaler = CGM.FLASH_DCR.B.DIV + 1uL;
			break;
		
		
		case PERIPHERAL_CLOCK_CANS:
			prescaler = 1uL;
			break;
		
		case PERIPHERAL_CLOCK_SIU:
			 prescaler = 1uL;
			break;
			
		case PERIPHERAL_CLOCK_WKUP:
			 prescaler = 1uL;
			break;
		
		case PERIPHERAL_CLOCK_FEC_0:
			prescaler = CGM.FEC_DCR.B.DIV + 1uL;
			break;
		
		case PERIPHERAL_CLOCK_FLEXRAY_0:
			if( CGM.AC1_DC0.B.DE0){
				prescaler = CGM.AC1_DC0.B.DIV0 + 1;
				}
			/* Check what system clock that is selected */
            switch( CGM.AC1_SC.B.SELCTL ) {
                 case 1:
...
#elif defined(CFG_MPC5646B)
	/* PFCR0.B02_RWSC:
	0 MHz �20 MHz, APC =RWSC=0
	> 20 MHz �40 MHz, APC =RWSC=1
	> 40 MHz �60 MHz, APC =RWSC=2
	> 60 MHz �80 MHz, APC =RWSC=3
	> 80 MHz �100 MHz, APC =RWSC=4
	>100 MHz �120 MHz, APC =RWSC=5
	----------------------------------------------
    Default settings should be fine for 120MHz system clock.
    Otherway modification of these registers should to be executed from RAM
    NOT from FLASH ...

    User could configure it better in startup code more convinient & safe way,
    in advance of setting PLL.
	Therefore we skip it.

	Addresses of PFLASH:
	CFLASH 0 = 0x00 0000 - 0x17 FFFF
	CFLASH 1 = 0x18 0000 - 0x2F FFFF

	Example:
	CFLASH0.PFCR0.R = 0xxxxxxxxxxxxxx;
    CFLASH0.PFCR1.R = 0xxxxxxxxxxxxxx;
	*/

#warning If SystemClk is <= 100MHz, then set Wait states in startup code appropriately from RAM code segment.
#warning After setting of FLASH/RAM WS cleaninig of cache is recommended.
...
#error No flash setup for this MCU
	...
	/* Enable Flash Non-Correctable error reporting */
     ECSM.ECR.B.EFNCR = 1uL;
	/* Enable RAM Non-Correctable error reporting */
     ECSM.ECR.B.ERNCR = 1uL;
#warning If Sigle bit corrections REPORTING for RAM/FLASH is needed, then user should enable it in startup code.
#elif defined(CFG_MPC5643L) || defined(CFG_SPC56XL70)
...
void Mcu_Arc_WdgExtendTimeout( void ) {
    ...
    /*lint -save -e923  Ok, this cast is in Freescale header file */
#if defined(CFG_MPC560X) || defined(CFG_MPC563XM) || defined(CFG_MPC5643L) || defined(CFG_MPC5645S) || defined(CFG_MPC5646B) || defined(CFG_SPC56XL70) || defined(CFG_MPC5644A) || defined(CFG_MPC5668)
...
#elif defined(CFG_MPC5744P) || defined(CFG_MPC5643L) || defined(CFG_SPC56XL70) || defined(CFG_MPC5777M) || defined(CFG_MPC5777C) || defined(CFG_MPC5668)
