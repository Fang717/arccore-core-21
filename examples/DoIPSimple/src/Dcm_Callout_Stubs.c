...
__attribute__ ((section (".bootMsg1"))) uint32 bootMsgProgSig[2];
...
#if defined(CFG_BOOTLOADER_APPL)
            bootMsgProgSig[0] = 0x50726f67;
            bootMsgProgSig[1] = 0x5369676E;

            (void)SoAd_CloseSoCon(SOAD_SOCON_ID_SOADSOCKETCONNECTIONGROUP_DOIP_TCP_SOADSOCKETCONNECTION_DOIP_TCP, FALSE);
            SoAd_MainFunction();

            // Let communication channel shutdown gracefully by letting TcpIp run for a little while
