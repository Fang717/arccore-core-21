...
Std_ReturnType Rte_Switch_WdgM_wdgm_mode_Supervised100msTask_currentMode(/*IN*/uint8 mode) {
    if (WdgM_ModeMachines.wdgm.mode_Supervised100msTask_currentMode.nextMode == RTE_TRANSITION_WdgM_wdgm_mode_Supervised100msTask_currentMode) {
        {
            SYS_CALL_SuspendOSInterrupts();
            WdgM_ModeMachines.wdgm.mode_Supervised100msTask_currentMode.nextMode = mode;
            SYS_CALL_ResumeOSInterrupts();
        }
        // Activate runnables
