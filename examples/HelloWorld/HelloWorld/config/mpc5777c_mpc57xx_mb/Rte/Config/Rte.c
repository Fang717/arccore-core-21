...
extern ComM_ModeMachinesType ComM_ModeMachines; /*lint -e9003 RTE Requirement */
...
void OsRteTask(void) { /** @req SWS_Rte_02251 */
    ...
    do {
        SYS_CALL_WaitEvent(
                EVENT_MASK_ModeSwitchOsEvent | EVENT_MASK_SwcMemCmdReceived | EVENT_MASK_WatchdogModeSwitchEvent | EVENT_MASK_OsEvent2
                        | EVENT_MASK_ComMFullComOsEvent | EVENT_MASK_OsEvent1); /*lint !e534 RTE Requirement */
        ...
        if ((Event & (EVENT_MASK_WatchdogModeSwitchEvent)) != 0) {
            ...
            if (WdgM_ModeMachines.wdgm.localMode_Supervised100msTask_currentMode.nextMode
                ...
                WdgM_ModeMachines.wdgm.localMode_Supervised100msTask_currentMode.previousMode =
                        WdgM_ModeMachines.wdgm.localMode_Supervised100msTask_currentMode.currentMode;
                WdgM_ModeMachines.wdgm.localMode_Supervised100msTask_currentMode.currentMode =
                        RTE_TRANSITION_WdgM_wdgm_localMode_Supervised100msTask_currentMode; /* Indicate ongoing transition */

            }
        }
        if ((Event & (EVENT_MASK_ComMFullComOsEvent)) != 0) {
            /* Check that a switch has been requested (nextMode is not a transition) */
            if (ComM_ModeMachines.comM.UM_ComMUser_currentMode.nextMode != RTE_TRANSITION_ComM_comM_UM_ComMUser_currentMode) {
                ComM_ModeMachines.comM.UM_ComMUser_currentMode.previousMode = ComM_ModeMachines.comM.UM_ComMUser_currentMode.currentMode;
                ComM_ModeMachines.comM.UM_ComMUser_currentMode.currentMode = RTE_TRANSITION_ComM_comM_UM_ComMUser_currentMode; /* Indicate ongoing transition */

            }
        }
        if ((Event & EVENT_MASK_SwcMemCmdReceived) != 0) {
            SYS_CALL_ClearEvent (EVENT_MASK_SwcMemCmdReceived);/*lint !e534 RTE Requirement */
            Rte_SwcMem_Main();
        }
        if ((Event & EVENT_MASK_OsEvent2) != 0) {
            SYS_CALL_ClearEvent (EVENT_MASK_OsEvent2);/*lint !e534 RTE Requirement */
            Rte_SwcReader_SwcReaderRunnable();
        }
        if ((Event & EVENT_MASK_OsEvent1) != 0) {
            SYS_CALL_ClearEvent (EVENT_MASK_OsEvent1);/*lint !e534 RTE Requirement */
        ...
        if ((Event & (EVENT_MASK_WatchdogModeSwitchEvent)) != 0) {
            /* Check that a transition is ongoing */
            if (WdgM_ModeMachines.wdgm.localMode_Supervised100msTask_currentMode.currentMode
                    == RTE_TRANSITION_WdgM_wdgm_localMode_Supervised100msTask_currentMode) {

                /* Activate runnables ON-ENTRY */
                if (WdgM_ModeMachines.wdgm.localMode_Supervised100msTask_currentMode.nextMode
                        == RTE_MODE_WdgM_wdgm_localMode_Supervised100msTask_currentMode_SUPERVISION_FAILED) {
                    Rte_SwcReader_SwcReaderRunnable();
                }

                WdgM_ModeMachines.wdgm.localMode_Supervised100msTask_currentMode.currentMode =
                        WdgM_ModeMachines.wdgm.localMode_Supervised100msTask_currentMode.nextMode;
                WdgM_ModeMachines.wdgm.localMode_Supervised100msTask_currentMode.nextMode =
                        RTE_TRANSITION_WdgM_wdgm_localMode_Supervised100msTask_currentMode; /* Indicate that no transition is requested (nextMode can be a transition) */
                WdgM_ModeMachines.wdgm.localMode_Supervised100msTask_currentMode.transitionCompleted = TRUE;
            }
        }
        if ((Event & (EVENT_MASK_ComMFullComOsEvent)) != 0) {
            /* Check that a transition is ongoing */
            if (ComM_ModeMachines.comM.UM_ComMUser_currentMode.currentMode == RTE_TRANSITION_ComM_comM_UM_ComMUser_currentMode) {

                /* Activate runnables ON-ENTRY */
                if (ComM_ModeMachines.comM.UM_ComMUser_currentMode.nextMode == RTE_MODE_ComM_comM_UM_ComMUser_currentMode_COMM_FULL_COMMUNICATION) {
                    Rte_SwcWriter_FullCommunication();
                }

                ComM_ModeMachines.comM.UM_ComMUser_currentMode.currentMode = ComM_ModeMachines.comM.UM_ComMUser_currentMode.nextMode;
                ComM_ModeMachines.comM.UM_ComMUser_currentMode.nextMode = RTE_TRANSITION_ComM_comM_UM_ComMUser_currentMode; /* Indicate that no transition is requested (nextMode can be a transition) */
                ComM_ModeMachines.comM.UM_ComMUser_currentMode.transitionCompleted = TRUE;
            }
        }
        ...
        /* Clear event set by ModeSwitch */
        if ((Event & EVENT_MASK_ComMFullComOsEvent) != 0) {
            SYS_CALL_ClearEvent (EVENT_MASK_ComMFullComOsEvent); /*lint !e534 RTE Requirement */
        }

        EcuM_ModeMachines.ecuM.currentMode_currentMode.transitionCompleted = FALSE;
        WdgM_ModeMachines.wdgm.localMode_Supervised100msTask_currentMode.transitionCompleted = FALSE;
