...
Std_ReturnType BswM_WdgMEnableActionActionHandle( const BswM_ActionListItemType *item );
Std_ReturnType BswM_WdgMDisableActionActionHandle( const BswM_ActionListItemType *item );
...
boolean BswM_ComMSilentCommuncationlExpressionLogicalExpHandle( void );
boolean BswM_ComMNoCommuncationlExpressionLogicalExpHandle( void );
boolean BswM_WdgMModeExpressionLogicalExpHandle( void );
...
Std_ReturnType BswM_WdgMEnableActionActionHandle( const BswM_ActionListItemType *item ) {
	...
	(void)WdgM_SetMode(WdgMConf_WdgMMode_WatchdogOn);
...
Std_ReturnType BswM_WdgMDisableActionActionHandle( const BswM_ActionListItemType *item ) {
	...
	(void)WdgM_SetMode(WdgMConf_WdgMMode_WatchdogOff);
...
boolean BswM_ComMSilentCommuncationlExpressionLogicalExpHandle( void ) {
	return (((ComM_ModeType) BswM_ModeReqMirrors[BSWM_BSWCOMMINDICATION_REQ_IDX] == COMM_SILENT_COMMUNICATION));
}

boolean BswM_ComMNoCommuncationlExpressionLogicalExpHandle( void ) {
	return (((ComM_ModeType) BswM_ModeReqMirrors[BSWM_BSWCOMMINDICATION_REQ_IDX] == COMM_NO_COMMUNICATION));
}

boolean BswM_WdgMModeExpressionLogicalExpHandle( void ) {
	return (((uint8) BswM_ModeReqMirrors[BSWM_WDGMMODE_REQ_IDX] == SUPERVISION_OK));
}

Std_ReturnType (*BswM_AtomicActions[BSWM_NUMBER_OF_ATOMIC_ACTIONS]) ( const BswM_ActionListItemType *item ) = 
{
	[BSWM_IPDUALLENABLED_ACTION_IDX] = BswM_PduGroupSwitchActionHandle,
	[BSWM_REQUESTFULLCOM_ACTION_IDX] = BswM_RequestFullComActionHandle,
	[BSWM_IPDULISTENONLY_ACTION_IDX] = BswM_PduGroupSwitchActionHandle,
	[BSWM_IPDUALLDISABLED_ACTION_IDX] = BswM_PduGroupSwitchActionHandle,
	[BSWM_WDGMENABLEACTION_ACTION_IDX] = BswM_WdgMEnableActionActionHandle,
	[BSWM_WDGMDISABLEACTION_ACTION_IDX] = BswM_WdgMDisableActionActionHandle,
};
...
boolean (*BswM_LogicalExpressions[BSWM_NUMBER_OF_LOGICAL_EXPRESSIONS]) ( void ) = 
{
	...
	[BSWM_COMMSILENTCOMMUNCATIONLEXPRESSION_EXP_IDX] = BswM_ComMSilentCommuncationlExpressionLogicalExpHandle,
	[BSWM_COMMNOCOMMUNCATIONLEXPRESSION_EXP_IDX] = BswM_ComMNoCommuncationlExpressionLogicalExpHandle,
	[BSWM_WDGMMODEEXPRESSION_EXP_IDX] = BswM_WdgMModeExpressionLogicalExpHandle,
};
...
static inline uint8 BswM_Internal_Read_SwcModeReq_WdgMMode( void ) {
	uint8 tmp;
	(void) Rte_Read_modeRequestPort_WdgMMode_requestedMode( &tmp );
	return tmp;
}
...
void BswM_Internal_UpdateNotificationMirrors( void ) {
	...
	BswM_ModeReqMirrors[BSWM_WDGMMODE_REQ_IDX] = (uint32) BswM_Internal_Read_SwcModeReq_WdgMMode();
...
void BswM_CanSM_CurrentState( NetworkHandleType Network,
	...
	BSWM_VALIDATE_INIT_NORV(BSWM_SERVICEID_CANSMCURRENTSTATE); /* @req BswM0080 */
