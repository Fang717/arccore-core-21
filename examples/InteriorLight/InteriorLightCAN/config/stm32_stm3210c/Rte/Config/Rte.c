...
void OsRteTask(void) { /** @req SWS_Rte_02251 */
    ...
    do {
        SYS_CALL_WaitEvent(EVENT_MASK_OsMainEvent | EVENT_MASK_OsInitEvent); /*lint !e534 RTE Requirement */
        SYS_CALL_GetEvent(TASK_ID_OsRteTask, &Event); /*lint !e534 RTE Requirement */

        if ((Event & (EVENT_MASK_OsInitEvent)) != 0) {
        ...
        if ((Event & EVENT_MASK_OsMainEvent) != 0) {
            SYS_CALL_ClearEvent (EVENT_MASK_OsMainEvent);/*lint !e534 RTE Requirement */
        ...
        if ((Event & EVENT_MASK_OsInitEvent) != 0) {
            SYS_CALL_ClearEvent (EVENT_MASK_OsInitEvent); /*lint !e534 RTE Requirement */
        }
